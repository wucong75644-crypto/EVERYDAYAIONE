...
BEGIN
  ...
  IF EXISTS (
      ...
      SELECT 1 FROM agent_runtime_tenant_provider_bindings
       WHERE catalog_revision=v_catalog_revision
  ) THEN
    RAISE EXCEPTION 'AGENT_RUNTIME_BATCH_MEDIA_RELEASE_FACTS_EXIST';
  END IF;
  DELETE FROM agent_runtime_production_bindings
   WHERE catalog_revision=v_catalog_revision;
