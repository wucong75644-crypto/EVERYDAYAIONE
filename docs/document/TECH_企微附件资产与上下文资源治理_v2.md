...
迁移 131 为任务冻结附件引用时保留了集合的 `active` 状态，导致同一集合被后续每条
企微文字重复拼入当前消息。迁移 133 将“当前附件”和“历史资源”重新分层：

- 未绑定任务的附件保持 `active`，等待下一条生成指令。
- 首次入队先写入不可变 `task_attachment_refs`，再把已绑定资产转为 `referenced`。
- 同一消息重放复用已经冻结的输入，不消费之后新上传的附件。
- 已有任务引用但仍为 `active` 的生产数据在迁移时修正；未使用附件不变。
- rollback 只恢复函数行为，不重新激活已消费历史资源。

`referenced` 不删除源消息、资产 ID、Workspace 文件或历史分析结果，只阻止历史附件
继续冒充后续消息的当前附件。Web 仍使用当前请求显式提交的附件列表，不经过此消费
函数。
