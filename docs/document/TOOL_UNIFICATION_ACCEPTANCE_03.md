...
> 最新追加：用户要求核清代码后修复的文件资源边界已完成。主组 1481 + 独立 ERP 11 = 1492 passed，4 原有 xfailed；41 个新边界/兼容用例覆盖删除零执行、正确目标、恢复记录、staging 与旧返回对象。当前范围、G-01～05、源码指纹和限制以 [边界修复验收](FILE_TOOL_BOUNDARY_REPAIR.md) 为准。旧 03/附件修复记录继续保留为阶段快照，未提交部署或用户验收关闭。

> 2026-09-09 追加：首次候选 `e243ba2c0d545d8afca3ae930a40389276b7c123` 已提交部署。用户随后报告附件 ID 无法读取，并明确授权在本工作树修复。当前为该候选加未提交的文件缓存修复，技术验证通过、待新候选部署/用户验收。新增授权范围、G-01～05、21 个前后对照场景和 1111 项回归见 [附件 ID 修复记录](FILE_CACHE_ID_REPAIR.md)。下文保留首次开发验收快照，不能将其中“生产源码无差异”“尚未部署”用于描述本次追加修复后的状态；当前验证入口为该修复记录中的脚本。
