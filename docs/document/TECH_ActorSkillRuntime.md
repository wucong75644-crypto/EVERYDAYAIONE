...
P2-3 已增加用户/组织管理员明确设置的会话预绑定，见 [Skill 会话预绑定](TECH_Skill会话预绑定.md)。新 Turn 先加载固定版本的会话绑定，模型激活仍只限本 Turn；恢复不读取当前 binding，权限继续取交集。下文“仅本 Turn”描述的是运行状态及模型激活范围。

P2-2 当前契约见 [Skill 资料附件与模板资产](TECH_Skill资料附件与模板资产.md)：新增 revision 资产白名单、预算内按正文引用读取、资产快照核验。新激活的模板参数改为显式声明且仅由服务端上下文提供；下文模型传入 args 的示例属于 P1-3 历史行为。
