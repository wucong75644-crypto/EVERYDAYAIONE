...
| `ScheduledWecomDeliveryRepositoryPort` / `PostgresScheduledWecomDeliveryRepository` | `backend/services/agent/runtime/ports/scheduled_wecom_delivery.py`、`infrastructure/postgres/scheduled_wecom_delivery.py` | Frozen typed boundary over claim v2, prepared recovery, 227_45 dispatch v2, 227_46 safe payload readback, 227_47 unsupported terminalization, 227_40 outcome and 227_41～44 reconciliation RPCs. Explicit parameters preserve request/worker/token/version/hash/revision fences; exact output/target allowlists reject malformed or sensitive fields, payload reads are not connection-replayed, and durable request-ledger mutations replay only their identical request after a lost response. |
| `ScheduledWecomSmartDispatchService.dispatch_claimed` / `scheduled_wecom_smart_identity` / `scheduled_wecom_receipt_hash` | `backend/services/agent/runtime/application/scheduled_wecom_smart_dispatch.py`、`scheduled_wecom_receipt.py` | Runtime-owned one-shot Smart Robot orchestration over an already claimed and verified typed payload. It rejects App or fence drift before persistence, registers a deterministic routed item/provider single-flight before awaiting prepare, and lets one shielded internal task own prepare→start→send→outcome so caller cancellation cannot release the flight or resend. Only fresh prepare/start owners send exactly `markdown/{content}` once; typed ACK/rejection maps to SQL-parity allowlisted evidence and every non-definitive post-start result records UNKNOWN. Transport/internal cancellation best-effort persists UNKNOWN then re-raises; failed persistence leaves `dispatch_started` for 227_48. It does not claim/read/terminalize, compose recovery, or activate a worker. |
| `ScheduledWecomAppDispatchService.dispatch_claimed` / `ScheduledWecomAppBinding` / `scheduled_wecom_app_identity` | `backend/services/agent/runtime/application/scheduled_wecom_app_dispatch.py`、`ports/scheduled_wecom_app_dispatch.py`、`scheduled_wecom_receipt.py` | Runtime-owned one-shot App orchestration over an already claimed App payload and an explicitly injected non-secret `org_id/corp_id/agent_id + WecomAppOutbound-compatible transport` binding. Exact org/corp route validation precedes all persistence; a domain-separated item/payload/provider/binding identity registers single-flight before prepare, and one shielded owner performs fresh prepare→start→one typed text send→durable outcome. ACK and direct/partial provider rejection retain only allowlisted message-id/errcode evidence with SQL-parity hashes; every non-definitive post-start result records UNKNOWN, while failed cancellation persistence remains recoverable by 227_48. It does not resolve config/credentials, claim/read payload, call legacy senders, compose routing, or activate production. |
| `ScheduledWecomRouter.dispatch_once` / `AppBindingResolverPort.resolve_app_binding` | `backend/services/agent/runtime/application/scheduled_wecom_router.py`、`ports/scheduled_wecom_router.py`、`ports/scheduled_wecom_app_dispatch.py` | Runtime-owned single-pass claim→one safe payload read→typed routing boundary. Same-request single-flight prevents duplicate local transport; unsupported items use one deterministic 227_47 request, unavailable/fenced/config-missing paths create no attempt, Smart routes to the existing Smart service, and App resolves only an exact non-secret tenant binding before the existing App service. Configuration decryption, concrete Worker composition, prepared/started recovery and UNKNOWN reconciliation remain separate next-batch owners. |
| `ScheduledWecomAppBindingResolver.resolve_app_binding` | `backend/services/wecom/scheduled_app_binding.py` | Trusted exact-tenant adapter over the 227_50 `wecom.app` façade. It creates an actorless exact-org `WORKER` DB scope, strictly validates organization-only corp/agent/secret/version facts, keeps Secret inside an exact-match production broker backend and lease consumer, reuses the injected per-org token manager plus shared outbound HTTP client, and exports only `ScheduledWecomAppBinding`. Database, material service, token manager, HTTP client and credential audit sink are all mandatory; failures close to `None` while cancellation propagates. |
