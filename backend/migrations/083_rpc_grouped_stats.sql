-- ============================================================
-- 083: 分组统计 RPC + not_like 操作符 + 白名单扩展
--
-- 1. 新增 erp_order_stats_grouped RPC — 按 order_type + order_status 分组
-- 2. erp_global_stats_query 新增 not_like 操作符 + 白名单扩展
--
-- ⚠️ 两个 RPC 的 Filter DSL 白名单必须同步维护！
--    修改白名单时搜索 "FILTER_WHITELIST_SYNC" 找到两处。
--
-- 设计文档: docs/document/TECH_ERP数据完整性与查询准确性.md §5.5, §5.7
-- ============================================================


-- ╔══════════════════════════════════════════════════════════╗
-- ║  1. erp_global_stats_query — 新增 not_like + 白名单扩展  ║
-- ╚══════════════════════════════════════════════════════════╝
...
COMMENT ON FUNCTION erp_global_stats_query IS
    'ERP全局统计RPC（多租户），支持 Filter DSL + not_like 操作符';


-- ╔══════════════════════════════════════════════════════════╗
-- ║  2. erp_order_stats_grouped — 按 order_type + status 分组 ║
-- ╚══════════════════════════════════════════════════════════╝
...
COMMENT ON FUNCTION erp_order_stats_grouped IS
    '订单分组统计RPC — 按 order_type + order_status + is_scalping 分组，供分类引擎消费';
