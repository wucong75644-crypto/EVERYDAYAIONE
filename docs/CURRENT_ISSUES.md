...
- 2026-09-21 / 数据对比表显示 HTML 标签（本地已修复，未部署）：模型在 Markdown 表格中用 `span style` 为涨跌幅上色，默认渲染将标签转为文字。现仅在 Markdown 行内容器中移除 `span` 包装，保留全部数值、箭头与 Markdown；代码示例、转义文字和完整 HTML 块仍保留原文，不启用 HTML 执行。聊天提示补充受支持的输出格式。截图 8 行数据回归及流式、代码、安全边界已覆盖，新增用例在修复前复现 4 项失败；修复后前端相关 55 项、后端模板 8 项、定向 ESLint、类型检查与生产构建通过（现有大分包提示仍在）。未修改计算、持久化数据或生产环境。见 [渲染规则](document/TECH_结构化消息运行时边界与渲染安全.md#33-渲染规则)。
