...
ERP_ROUTING_PROMPT = (
    ...
    "## 易混淆场景决策（P0 高频）\n"
    "### 库存查询（按场景选）\n"
    "- 「XX商品库存多少」→ erp_product_query(stock_status, outer_id=XX) 用主商家编码\n"
    "- 「XX规格/SKU库存多少」→ erp_product_query(stock_status, sku_outer_id=XX) 用规格编码\n"
    "- 「这个商品在各仓库分别多少」→ erp_product_query(warehouse_stock, outer_id=XX 或 sku_outer_id=XX) 按仓拆分\n"
    ...
    "- ⚠️ outer_id（主商品编码）和 sku_outer_id（规格编码）是不同参数！\n"
    "  用户说「规格编码」「SKU编码」→ 一定用 sku_outer_id\n"
    "  编码末尾带 -数字 后缀（如 NXMWY01-02、DBTXL01-02）→ 整个编码就是 sku_outer_id，直接传完整编码，禁止拆分\n"
    "  stock_status 和 warehouse_stock 都支持 sku_outer_id，可直接查某个规格的库存\n\n"
    "### 售后查询（5种，跨3个工具）\n"
    ...
    "- 按outer_id查库存/商品无结果 → 改用 sku_outer_id 重试（可能是规格编码）\n"
    "- 按sku_outer_id查无结果 → 改用 outer_id 重试（可能是主商品编码）\n"
    ...
    "用户：「NXMWY01-02库存多少」\n"
    "→ 编码末尾 -02 → 规格编码\n"
    "→ erp_product_query(action=\"stock_status\", sku_outer_id=\"NXMWY01-02\")\n\n"
    "用户：「最近7天的退货单」\n"
    ...
    "- 英文字母+数字混合 → 商家编码，按以下规则区分：\n"
    "  - 用户明确说「规格编码」「SKU编码」→ sku_outer_id\n"
    "  - 编码末尾带 -数字 后缀（如 NXMWY01-02、DBTXL01-02）→ 整个编码作为 sku_outer_id 传入，不要拆分\n"
    "  - 无 -数字 后缀（如 ABC123、NXMWY01、HM-2026）→ 主商品编码 outer_id\n"
    "  - 不确定 → 先当 outer_id 查，无结果自动改 sku_outer_id 重试\n"
