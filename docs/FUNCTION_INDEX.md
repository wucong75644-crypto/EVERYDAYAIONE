...
| `claim_agent_runtime_scheduled_wecom_delivery_v1` / `renew_agent_runtime_scheduled_wecom_delivery_lease_v1` / claim and dispatch-context reads | `backend/migrations/227_38_agent_runtime_scheduled_wecom_claim.sql` | WeCom delivery worker (`session_user=everydayai_wecom_runtime`, `app.access_kind=worker`) gets only request-id readback, SKIP LOCKED claim, lease renewal and a fenced safe dispatch gate. Claim readback is side-effect-free; dispatch-context evaluation revalidates each frozen D1/A1 identity and may atomically mark a pre-dispatch unavailable target and cancel its pending items. |
