...
| `KieClient.create_task` / `_schedule_shadow_upload` | `backend/services/adapters/kie/client.py` | CDN首次生成，受理后仅后台启动海外旁路：继承7890下载CDN、7891上传；逐张按序缓存完整临时链接供400单次兜底，首次任务不等旁路 | request / request, task_id | CreateTaskResponse / None |
