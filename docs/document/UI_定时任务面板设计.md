...
## 十四、V3：可见的创建与执行状态（2026-08-29）

定时任务的关键风险不是“用户不知道按钮在哪里”，而是把“表单已生成”或“预检完成”误解为“正式任务已经创建”。因此网页面板与聊天表单使用同一组四阶段状态，并在每一步显示不可越过的边界：

| 阶段 | 用户可见状态 | 允许的操作 | 必须明确说明 |
|------|-------------|-----------|--------------|
| 1. 填写配置 | `尚未创建任务` | 修改配置、开始规划与试跑 | 点击主按钮不会直接创建任务 |
| 2. 规划与试跑 | 加载中的路径验证 | 等待或返回修改 | 只读；不扣积分、不发送消息、不写入业务数据 |
| 3. 确认启用 | 计划、工具路径、预检结论 | 确认启用或修改后重试 | 预检通过不等于已创建，确认才创建正式任务 |
| 4. 已启用 | 执行历史与实际工具轨迹 | 查看、暂停、立即执行 | 成功须同时满足工具成功和完整业务结论；失败/超时不得显示成功 |

聊天表单必须与面板使用一致的状态文案。AI 在返回第 1 阶段表单后只能说“配置表单已生成，任务尚未创建”，不得自行补写频率或声称任务已生效。第 3 阶段的确认表单必须展示已通过的工具路径；第 4 阶段则在执行历史中展示实际路径、完成门禁与失败原因。

验收：从聊天或面板进入任一创建路径时，用户都能看到当前阶段；在第 1、2、3 阶段均不会出现“任务已创建/已生效”的文案；正式执行失败时，执行历史包含失败原因与工具轨迹，而不是成功提示。

---

## 十五、ChangeSet 操作语义（2026-08-31）

ChangeSet 是所有任务变更的审计、幂等和版本保护边界，但不代表每个操作都要展示“AI 规划与试跑”。面板必须按操作的用户意图呈现：

| 操作 | 交互 | 不应出现的内容 |
|------|------|----------------|
| 创建、编辑 | 配置表单 → AI 规划与只读试跑 → Diff → 确认提交 | 把方案误说成已生效 |
| 暂停、恢复 | 状态变化与提交前版本/权限检查 → 确认 | AI 规划、只读试跑 |
| 删除 | 明确列出目标任务与不可恢复提示 → 危险确认 | 配置、Diff 或 AI 规划 |
| 立即执行 | 直接异步触发一次执行，并在历史中反馈结果 | ChangeSet 确认流程 |

所有确认仍从服务端 ChangeSet 当前状态读取。重复点击必须禁用；版本变化时显示重新规划提示；取消不得改变任务。
