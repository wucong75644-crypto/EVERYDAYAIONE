...
- **2026-07-17**：Web 图表正常但企业微信静默丢弃 `ChartPart`。治理方案改为通道末端适配：Web 保留交互式 ECharts；企微 Outbox 识别 chart，使用禁网 Playwright/Chromium 和锁定 ECharts runtime 渲染 PNG，再按智能机器人官方 `init/chunk/finish` WebSocket 素材协议获取 `media_id` 并发送原生 image。发送成功后才提交逐项检查点；部署增加真实 Chromium→PNG smoke gate。开发验证完成，尚未提交部署。
