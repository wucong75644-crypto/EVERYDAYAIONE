-- AR-18-A1.2-B1: direct root Run creation observes task cancel intents.
...
BEGIN
    ...
    v_intent := _lock_agent_runtime_task_cancel_intent(p_command_id);
    ...
    IF v_intent.id IS NOT NULL THEN
        IF v_command.result_entity_id IS NULL
           OR v_intent.status IS DISTINCT FROM 'applied'
           OR v_intent.run_id IS DISTINCT FROM v_command.result_entity_id THEN
        ...
        IF NOT FOUND OR v_run.command_id IS DISTINCT FROM p_command_id
           OR v_run.session_id IS DISTINCT FROM p_session_id
           OR v_run.id IS DISTINCT FROM v_intent.run_id
           OR v_run.status IS DISTINCT FROM 'cancelled'
    ...
    IF FOUND THEN
        IF v_run.request_hash IS DISTINCT FROM v_request_hash THEN
            RETURN jsonb_build_object(
                'outcome', 'idempotency_conflict', 'entity_id', v_run.id);
        END IF;
        RETURN jsonb_build_object(
            'outcome', 'already_exists', 'entity_id', v_run.id,
            'state_version', v_run.state_version);
