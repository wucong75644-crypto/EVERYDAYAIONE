...
_DESCRIPTION_WORKSPACE = (
    ...
    "数据分析工作流：\n"
    "1. 先调 file_list 获取文件列表（每个文件含 abs 绝对路径）\n"
    "2. 用 abs 路径读表头: pd.read_excel(abs_path, engine='calamine', nrows=5)\n"
    ...
    "⚠ 禁止事项：\n"
    "- 禁止自己拼接文件路径 — 必须用 file_list 返回的 abs 路径\n"
