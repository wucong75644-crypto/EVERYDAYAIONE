...
每个工具背后是独立专家。专家只负责取数据，
怎么解读、怎么回复用户是你的事。

专家看不到你的对话历史——
你传的 task 和 conversation_context 是它理解任务的全部信息。
传多了，专家会把多余的词当成查询条件，取回来的数据是错的。
专家无法区分你加的词是展示要求还是过滤条件——
"刷单数"会变成"只查刷单"，"取消订单数"会变成"只查取消的"。
传少了，专家不知道该查什么。

所以 task 只传用户要查什么：
用户这次说了什么就传什么，只把模糊时间变成日期、指代词解析成具体名称。
怎么分析、呈现、对比——这些是你拿到数据后自己做的事，不要塞进 task。

conversation_context 是专家了解上文的唯一通道。
用户追问时，传上轮的查询条件，让专家能理解"再看拼多多的"。
不要放上轮的结果数字或你的推测，那些会变成错误的查询条件。
