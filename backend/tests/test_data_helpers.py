...
class TestSafeFloat:
    ...
    def test_none_returns_default(self):
        from services.sandbox.data_helpers import safe_float
...
class TestSafeInt:
    def test_safe_int_truncates(self):
        from services.sandbox.data_helpers import safe_int
        assert safe_int(1.7) == 1
        assert safe_int("1.7") == 1

    def test_safe_int_percentage(self):
        from services.sandbox.data_helpers import safe_int
        assert safe_int("47.40%") == 0  # 0.474 → 0

    def test_safe_int_placeholder(self):
        from services.sandbox.data_helpers import safe_int
        assert safe_int("-") == 0
        assert safe_int("无") == 0
...
class TestSandboxInjection:
    def test_helpers_injected_to_globals(self):
        """沙盒 globals 必须含 safe_float / safe_int"""
        from services.sandbox.data_helpers import install_data_helpers_in_globals
        g = {}
        install_data_helpers_in_globals(g)
        assert "safe_float" in g
        assert "safe_int" in g
        assert callable(g["safe_float"])
        assert callable(g["safe_int"])

    def test_helpers_callable_from_sandbox(self):
        """LLM 在沙盒里能直接调"""
        from services.sandbox.data_helpers import install_data_helpers_in_globals
        g = {}
        install_data_helpers_in_globals(g)
        assert g["safe_float"]("47.40%") == pytest.approx(0.474)
        assert g["safe_int"]("47.40%") == 0


# ============================================================
# 4. code_execute description 含 DATA HELPERS 段
# ============================================================

class TestCodeExecuteDescription:
    """LLM 看 code_execute description 时,必须能知道 safe_float 存在"""

    def _desc(self):
        from config.code_tools import build_code_tools
        return build_code_tools()[0]["function"]["description"]

    def test_description_mentions_safe_float(self):
        assert "safe_float" in self._desc(), \
            "code_execute description 必须声明 safe_float 存在"

    def test_description_mentions_safe_int(self):
        assert "safe_int" in self._desc()

    def test_description_has_data_helpers_section(self):
        assert "DATA HELPERS" in self._desc(), \
            "description 必须有 DATA HELPERS 段名称"

    def test_description_mentions_pct_example(self):
        """description 必须给百分比转换例子,LLM 才知道用法"""
        assert "47.40%" in self._desc() and "0.474" in self._desc(), \
            "description 必须含 '47.40% → 0.474' 例子展示"


# ============================================================
# 5. file_ai_prompt 含 step 6-10 (AI 识别 ragged/占位符/公式/日期 serial/同名列)
# ============================================================
