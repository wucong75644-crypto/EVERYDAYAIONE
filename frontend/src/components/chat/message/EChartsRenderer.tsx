...
import { useEChartsRender, type ChartInstance } from './useEChartsRender';
...
function EChartsRendererInner({ option, title, messageId }: EChartsRendererProps) {
  ...
  const hasOption = Object.keys(stableOption).length > 0;
  const [activeType, setActiveType] = useState<SwitchableType>(originalType || 'bar');
  const renderOption = useMemo(() => injectDefaults(
    canSwitch && chartData
      ? buildOption(stableOption, activeType, chartData)
      : stableOption,
  ), [activeType, canSwitch, chartData, stableOption]);

  const { status, error, retry } = useEChartsRender({
    containerRef,
    chartRef,
    option: renderOption,
    activeType,
...
export default memo(EChartsRendererInner);
