...
The current AR-17.3 remediation adds additive 226_08–226_15 lanes for strict fact idempotency, application-owned atomic provider/cost/ActionResult finalization, non-terminal reconciliation lease release, Child Run v2 readback/terminal aggregation, cancel parity, database-fact-based ERP sync recovery with ownership/version fencing, and exact worker RPC numeric overloads. The isolated PostgreSQL harness now drives the formal ActionLoop/Resolver/SpecialistExecutor/Postgres repository chain. Production activation remains unchanged and AR-17.3 is not accepted until the complete end-to-end matrix is closed.
