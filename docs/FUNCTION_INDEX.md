...
| `_agent_runtime_scheduled_delivery_normalize` / delivery snapshot and intent triggers | `backend/migrations/227_35_agent_runtime_scheduled_delivery_intents.sql` | Validate and deterministically flatten the existing web/WeCom/multi target shapes at submission, bind the later scheduled Runtime Run, and atomically append one pending, Secret-free intent per frozen target when finalization v2 applies; legacy runs never enter this path. |
| `read_agent_runtime_scheduled_delivery_intents_v1` | `backend/migrations/227_35_agent_runtime_scheduled_delivery_intents.sql` | Projection-worker-only tenant/scheduled-run/runtime-run readback of immutable target and terminal content identities; no claim, send, ACK, cancel, mutation or payload body is exposed in D1-A. |
