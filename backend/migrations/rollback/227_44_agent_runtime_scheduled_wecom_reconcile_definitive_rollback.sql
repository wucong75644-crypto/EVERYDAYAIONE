-- Roll back 227_44 only while no definitive reconciliation fact exists.

SET LOCAL ROLE everydayai_owner;

LOCK TABLE agent_runtime_scheduled_wecom_reconcile_definitive_requests IN SHARE ROW EXCLUSIVE MODE;
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM agent_runtime_scheduled_wecom_reconcile_definitive_requests) THEN
  RAISE EXCEPTION 'AGENT_RUNTIME_SCHEDULED_WECOM_RECONCILE_DEFINITIVE_ROLLBACK_HAS_FACTS'
   USING ERRCODE='55000';
 END IF;
END $$;

REVOKE ALL ON FUNCTION record_agent_runtime_scheduled_wecom_reconcile_definitive_result_v1(
 UUID,UUID,UUID,UUID,UUID,UUID,TEXT,BIGINT,BIGINT,TEXT,TEXT,BIGINT,TEXT,TEXT,TEXT,TEXT,JSONB)
 FROM everydayai_wecom_runtime;
DROP FUNCTION record_agent_runtime_scheduled_wecom_reconcile_definitive_result_v1(
 UUID,UUID,UUID,UUID,UUID,UUID,TEXT,BIGINT,BIGINT,TEXT,TEXT,BIGINT,TEXT,TEXT,TEXT,TEXT,JSONB);
DROP FUNCTION _agent_runtime_scheduled_wecom_reconcile_definitive_json(
 agent_runtime_scheduled_wecom_reconcile_definitive_requests,TEXT);
DROP TRIGGER runtime_scheduled_wecom_reconcile_definitive_global_request_guard
 ON agent_runtime_scheduled_wecom_reconcile_definitive_requests;
DROP FUNCTION _agent_runtime_scheduled_wecom_reconcile_definitive_request_guard();
DROP TRIGGER runtime_scheduled_wecom_reconcile_definitive_immutable
 ON agent_runtime_scheduled_wecom_reconcile_definitive_requests;
DROP FUNCTION _agent_runtime_scheduled_wecom_reconcile_definitive_immutable();
DROP TABLE agent_runtime_scheduled_wecom_reconcile_definitive_requests;
