...
# 中国时间（与 erp_document_items.doc_created_at 对齐）— 复用 utils.time_context 单一来源
...
async def local_db_export(
    ...
    cutoff_dt = now - timedelta(days=max(days, 1))
    ...
    # 时间事实头（导出时间 + 数据截止）
    try:
        date_range = DateRange.custom(cutoff_dt, now, reference=now)
        time_header = format_time_header(
            ctx=request_ctx, range_=date_range, kind="导出窗口",
        )
    except Exception:
        time_header = ""
