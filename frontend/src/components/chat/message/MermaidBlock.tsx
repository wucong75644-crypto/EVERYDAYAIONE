...
export default memo(function MermaidBlock({ children }: MermaidBlockProps) {
  ...
  if (error) {
    return (
      <div className="my-3 rounded-lg border border-error/20 bg-error-light p-4">
        <div className="text-xs text-error mb-2">Mermaid 图表语法错误</div>
        <pre className="text-sm text-text-secondary whitespace-pre-wrap">{children}</pre>
      </div>
    );
  }
  ...
  if (!svg) {
    return (
      <div className="my-3 flex items-center justify-center p-8 text-text-disabled text-sm">
        图表渲染中...
      </div>
    );
  }
