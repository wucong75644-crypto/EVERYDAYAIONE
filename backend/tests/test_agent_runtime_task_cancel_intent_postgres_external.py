...
def _seed(url: str, *, task_status: str = "pending") -> Case:
    ...
    with psycopg.connect(url) as conn:
        ...
        conn.execute(
            ...
            "VALUES(%s,%s,%s,'user',%s)",
            (conversation_id, USER, ORG, str(USER)),
        )
        conn.execute(
        ...
        conn.execute(
            ...
            "agent_definition_revision) VALUES(%s,%s,%s,%s,'user',%s,%s,'default','v1')",
            (session_id, conversation_id, ORG, USER, str(USER), USER),
        )
        conn.execute(
            ...
            (command_id, session_id, ORG, USER, key, Jsonb(payload), Jsonb(payload)),
        )
        conn.execute(
        ...
        conn.commit()
    return Case(task_id, output_id, session_id, command_id, key, envelope)
...
def _hash(url: str, case: Case, *, key: str | None = None) -> str:
    with psycopg.connect(url) as conn:
        conn.execute("SET LOCAL ROLE everydayai_owner")
        return conn.execute(
            "SELECT _agent_runtime_task_cancel_request_hash(%s,%s,%s,%s,%s,%s,%s)",
            (case.task_id, case.message_id, ORG, USER, case.session_id,
             case.command_id, key or case.idempotency_key),
        ).fetchone()[0]


def _facade(
    url: str, case: Case, *, request_hash: str | None = None,
    role: str = "everydayai_runtime", scope_user: UUID = USER,
    overrides: dict | None = None,
) -> dict:
    values = {
        "task": case.task_id, "message": case.message_id, "org": ORG,
        "user": USER, "session": case.session_id, "command": case.command_id,
        "key": case.idempotency_key,
        "hash": request_hash or _hash(url, case),
    }
    values.update(overrides or {})
    with _connect(url, role) as conn:
        _scope(conn, role, user=scope_user)
        return conn.execute(
...
def _scanner(url: str, worker_id: str) -> dict:
    return _worker(
...
def _verify_cancel_first(url: str) -> None:
    ...
    with psycopg.connect(url) as conn:
        ...
        with pytest.raises(psycopg.errors.ObjectNotInPrerequisiteState,
            ...
            conn.execute(
                "UPDATE agent_runtime_task_cancel_intents SET request_hash=%s "
                "WHERE id=%s", ("1" * 64, first["intent_id"]),
            )
        conn.rollback()
...
def _verify_root_run_races(url: str) -> None:
    facade_case = _seed(url)
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(pool.map(
            lambda fn: fn(),
            (lambda: _facade(url, facade_case),
             lambda: _direct_create(url, facade_case)),
        ))
    assert any(result["outcome"] in {"cancelled", "cancelled_before_claim"}
               for result in results)
    assert _run_count(url, facade_case.command_id) == 1

    root_case = _seed(url)
    with ThreadPoolExecutor(max_workers=2) as pool:
        list(pool.map(
            lambda fn: fn(),
            (lambda: _scanner(url, "root-race"),
             lambda: _direct_create(url, root_case)),
        ))
    assert _run_count(url, root_case.command_id) == 1
...
def _verify_binding_permissions_and_flags(url: str) -> None:
    ...
    with pytest.raises(psycopg.errors.InsufficientPrivilege,
                       match="TASK_CANCEL_BINDING_MISMATCH"):
        _facade(url, scoped, scope_user=uuid4())
