...
function PickerThumbnail({ item }: { item: PickerItem }) {
  const candidates = [
    toDisplayThumbnailUrl(item.thumbnail_url),
    toOriginalImageUrl(item.cdn_url),
    getWorkspacePreviewUrl(item.workspacePath),
  ...
  const [loadState, setLoadState] = useState({ key: candidateKey, index: 0 });
  const candidateIndex = loadState.key === candidateKey ? loadState.index : 0;
  const src = candidates[candidateIndex] || '';

  if (!src) {
    return <span className="text-xs text-[var(--s-text-tertiary)]">图片加载失败</span>;
  }
  return (
    <img
      src={src}
      onError={() => setLoadState({ key: candidateKey, index: candidateIndex + 1 })}
      alt={item.name}
