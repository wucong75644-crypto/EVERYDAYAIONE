...
CREATE FUNCTION link_agent_action_artifact(p_action_id UUID,p_attempt_id UUID,p_artifact_id UUID,p_role TEXT,p_parent_artifact_id UUID,p_content_hash TEXT,p_materialize_revision INTEGER,p_materialize_status TEXT,p_sensitivity TEXT) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
...
CREATE FUNCTION checkpoint_agent_artifact_materialization(p_action_id UUID,p_attempt_id UUID,p_artifact_id UUID,p_materialize_revision INTEGER,p_materialize_status TEXT) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
