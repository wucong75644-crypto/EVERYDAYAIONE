...
export default function ImageContextMenu({
  ...
  const items = [
    { label: '引用', icon: Quote, onClick: handleQuote, className: 'text-blue-600 dark:text-blue-400' },
    { label: '复制', icon: Copy, onClick: handleCopy, className: 'text-gray-700 dark:text-gray-300' },
    { label: '下载', icon: Download, onClick: handleDownload, className: 'text-gray-700 dark:text-gray-300' },
  ];
    ...
    >
      {items.map(({ label, icon: Icon, onClick, className }) => (
        <button
          key={label}
          onClick={onClick}
          className={`w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-2 ${className}`}
        >
