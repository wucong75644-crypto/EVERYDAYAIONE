...
> 2026-09-09 实现补充：工作区附件 ID 的生成协议保持不变，反查仍依赖会话文件缓存的登记完整性。先搜索再插入的登记遗漏及同名串文件已在当前任务工作树修复，尚待提交部署。当前反查使用 `FilePathCache.registered_paths()` 的唯一精确键；分析结果与附件状态按完整路径归属，不再按归一化文件名合并文件。详见 [修复方案与验收](FILE_CACHE_ID_REPAIR.md)。下文为原始设计与阶段实施记录。
