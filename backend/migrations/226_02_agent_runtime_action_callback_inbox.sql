...
CREATE FUNCTION record_agent_action_callback(
 ...
 p_payload_hash TEXT, p_payload_redacted JSONB,
 p_action_id UUID, p_attempt_id UUID
) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
DECLARE i agent_action_callback_inbox%ROWTYPE;
BEGIN
 PERFORM _assert_agent_runtime_actor(TRUE);
 IF p_action_id IS NULL OR p_attempt_id IS NULL
    OR p_payload_redacted ?| ARRAY['token','secret','password','cookie','authorization']
    OR p_payload_hash !~ '^[0-9a-f]{64}$' THEN RAISE EXCEPTION 'AGENT_CALLBACK_REJECTED' USING ERRCODE='22023'; END IF;
 IF NOT EXISTS (SELECT 1 FROM agent_action_attempts a WHERE a.id=p_attempt_id AND a.action_id=p_action_id
                AND a.callback_correlation=p_callback_correlation AND a.status IN ('dispatching','accepted','unknown')) THEN
...
END; $$;
REVOKE ALL ON FUNCTION record_agent_action_callback(TEXT,TEXT,TEXT,TEXT,JSONB,UUID,UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION record_agent_action_callback(TEXT,TEXT,TEXT,TEXT,JSONB,UUID,UUID) TO everydayai_agent_runtime_worker;
RESET ROLE;
