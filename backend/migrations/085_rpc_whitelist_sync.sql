-- ============================================================
-- 085: RPC Filter DSL 白名单同步 + 拒绝策略修复
--
-- 问题: Python 层 COLUMN_WHITELIST 有 37 个字段，RPC 层白名单只有 29 个。
--       缺失的字段（如 platform）在 filter DSL 中被静默丢弃，
--       导致查询条件不生效但上层无感知。
--
-- 修复:
-- 1. 两个 RPC 函数的 DSL 白名单补齐 8 个缺失字段
-- 2. 拒绝策略从 CONTINUE（静默跳过）改为返回错误信息
--
-- 影响: erp_global_stats_query / erp_order_stats_grouped
-- ============================================================


-- ╔══════════════════════════════════════════════════════════╗
-- ║  1. erp_global_stats_query — 白名单补齐 + 拒绝报错      ║
-- ╚══════════════════════════════════════════════════════════╝
...
BEGIN
    ...
    -- ── Filter DSL 解析 ─────────────────────────────────────
    -- FILTER_WHITELIST_SYNC: 与 erp_order_stats_grouped 保持同步
    IF p_filters IS NOT NULL AND jsonb_typeof(p_filters) = 'array' THEN
        FOR i IN 0..jsonb_array_length(p_filters) - 1 LOOP
            ...
            IF field_name NOT IN (
                ...
                -- 085 补齐：关联方 + 单据标识
                'platform', 'shop_name', 'supplier_name', 'warehouse_name',
                'doc_code', 'doc_id', 'express_company', 'item_name',
                -- 数值字段
                ...
                -- 085 修复：拒绝不在白名单的字段，返回错误而非静默跳过
                RETURN jsonb_build_object(
                    'error', format('filter 字段 "%s" 不在允许范围内', field_name)
                );
            END IF;
...
COMMENT ON FUNCTION erp_global_stats_query IS
    'ERP全局统计RPC（多租户），支持 Filter DSL + 白名单拒绝报错（085）';


-- ╔══════════════════════════════════════════════════════════╗
-- ║  2. erp_order_stats_grouped — 白名单补齐 + 拒绝报错      ║
-- ╚══════════════════════════════════════════════════════════╝
