...
管理员草稿页可添加、编辑、移除文本附件，随原审核流程发布；编辑区可直接选择并插入动态信息，同时生成有类型的服务端变量声明，高级设置保留查看和手动管理入口。已发布及历史版本显示附件摘要，接口只返回 `asset_summaries`，不返回附件正文、路径或哈希。平台包转管理草稿时保留原资产内容，之后修改只影响新 revision。模板配置错误及附件超预算以安全提示反馈，不回显底层路径。
