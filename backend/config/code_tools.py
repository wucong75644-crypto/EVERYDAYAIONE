...
_DESCRIPTION = (
    ...
    "预装 pandas/duckdb/plotly/openpyxl/pdfplumber/python-docx 等。\n"
    "\n"
    "STATEFUL KERNEL — 必读\n"
    ...
    "\n"
    "WHEN TO USE\n"
    "- 用户要图表/可视化(柱形图/折线图/饼图等) — 必须调,在 code 里用 plotly.express + fig.show()\n"
    ...
    "\n"
    "PLOT — 首选 plotly.express\n"
    "画图用 plotly.express + fig.show()。输出交互图(zoom/hover/pan),走 JSON spec 直传前端,不写盘。\n"
    "matplotlib / altair / emit_chart 链路也通(matplotlib 出 PNG;altair 出 vega-lite;emit_chart 出 ECharts JSON),按需选择。\n"
    "\n"
    "范本(直接抄,改字段):\n"
    "  import plotly.express as px\n"
    "  fig = px.bar(df, x='平台', y='订单数', title='2026-06-20 各平台订单数')\n"
    "  fig.show()   # 真调用,不要只 print 数据假装画了图\n"
    "\n"
    "数据特征 → API:\n"
    "  分类对比 → px.bar(长标签 orientation='h')\n"
    "  时间序列 → px.line\n"
    "  占比 ≤6 类 → px.pie\n"
    "  两数值 → px.scatter\n"
    "  分布 → px.histogram / px.box\n"
    "  多分类×值 → px.imshow(heatmap)\n"
    "\n"
...
CODE_ROUTING_PROMPT = (
    ...
    "\n"
    "## 图表(plotly.express 唯一)\n"
    "数据特征 → API:\n"
    "  时间+数值 → px.line       时间+多组 → px.line(color='系列')\n"
    "  分类+数值 → px.bar        长标签 → px.bar(orientation='h')\n"
    "  占比 ≤6 类 → px.pie       两数值 → px.scatter\n"
    "  分布 → px.histogram / px.box\n"
    "  多分类×值 → px.imshow(heatmap)\n"
