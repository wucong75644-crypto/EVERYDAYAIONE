...
## 最新状态：F08-05 文件搜索协议修复（2026-09-13）

前次确认/产物修复已以 `8428b72c` 提交部署。用户生产验收报告文件搜索失败，其余已测试项正常；原失败没有调用删除 Handler。本次在原任务工作树修复完整名字定位、显式模型 scope 与歧义传播，沿用原 fid/resource_ref、权限和删除确认。[实际改动、兼容性与证据](TOOL_UNIFICATION_FILE_PROTOCOL_FIX_08.md)。

本次为 `8428b72c` 加未提交差异；相关本地 1735 个不重复节点已有有效通过证据（含新增 29 项）。真实模型复验尚待新增调用授权，**F08-05 技术未验收通过；F08-03/F08-04 也未关闭。当前修复未提交部署、用户未验收、未受控关闭。** 以下为历史快照。


## 2026-09-13 整体验收缺陷修复（当前有效增量）

08 对 main `41732e4f` 的只读验收发现 F08-01 确认响应误批准/归属旁路、F08-02 缓存最终文件与图片重复；用户随后授权按最小方案修复。本任务分支 `codex/task/20260913102153-tool-unification-08-fixes`，R1 本地提交 `f5017b29`，R2 与下列记录组成最终本地候选，确定 SHA 见提交交付回执。

[修复范围、接口、逐项证据与用户验证单](TOOL_UNIFICATION_FIX_ACCEPTANCE_08.md)。新增确认 54 项和产物复用 14 项全部通过；本轮后端 3683 passed / 6 既有 skipped / 4 既有 xfailed，前端 153、类型检查通过；35 Handler 三入口 210 条权限矩阵、读重叠/写屏障、独立进程回放和临时 PG 均已复验。两个 Bug 的修复技术验证通过。

**整体验收仍未通过：U08-01 / ST-26 新模型协议六个真实合成样本未验证。当前未推送/部署/合并 main，用户生产验收及受控关闭未完成。** 无新业务能力、工具 schema/API/WS 字段、数据库迁移或收费策略。以下各阶段“当前/未关闭”保留为当时快照，不能覆盖本段或提交后的最终验收回执。
