...
ERP_ROUTING_PROMPT = (
    ...
    "- local_db_export：导出本地数据库明细到 staging 文件（毫秒级），配合 code_execute 生成 Excel。"
    "导出/报表/Excel 需求优先用此工具，不要用 fetch_all_pages 走远程API\n"
    "- fetch_all_pages：本地没有的数据（如物流轨迹）全量翻页拉取。本地有的数据禁止用\n"
    "- erp_* 远程工具：物流轨迹、操作日志、仓库操作、写入操作等 local 不支持的场景\n"
    "- code_execute：纯计算沙盒，读 staging 文件做计算/格式化/导出 Excel\n\n"
    "### 常见场景路由（优先匹配）\n"
    ...
    "禁止用 date 拉全天数据再估算\n"
    "- 含「对比/同比/环比/比上周/比上月/比去年」→ local_compare_stats，"
    "禁止调用 local_global_stats 两次再口述对比\n\n"
    "## 时间事实使用规范（强制）\n"
    "- 工具返回顶部的 [统计区间] / [当前期] / [基线期] / [查询窗口] 时间块，"
    "其中的日期+中文星期（如「2026-04-10 周五」）必须**逐字复述**，禁止重新格式化\n"
    "- 禁止自行推算「4月X日是周几」——直接读工具返回的中文星期\n"
    "- 禁止自行计算「上周X日是哪天」——必须用 local_compare_stats\n"
    "- 时间术语约定：上周 = ISO 周（周一为始）的上一周；同比 = 去年同期；环比 = 上一周期同位置\n\n"
    "### 降级策略\n"
    ...
    "- 同一编码每会话只识别一次\n\n"
    "## 歧义处理（强制）\n"
    "- local_product_identify 返回多条匹配 → 必须 ask_user 让用户选择，禁止自行取第一条\n"
    "- 店铺名/供应商名模糊匹配到多个 → 必须 ask_user 确认具体目标\n"
    "- 用户说「那个」「上次的」等指代不明 → ask_user 确认，不要猜\n"
    "- 只有精确匹配到唯一结果时才可直接继续查询\n\n"
    "## 时间类型（远程+本地通用，必须严格匹配）\n"
    ...
    "- 结果 ≤20 条明细 → 直接文字回复\n"
    "- 导出Excel/报表/全量数据 → local_db_export 导出到 staging → code_execute 生成 Excel\n"
    "- 本地没有的数据（如物流轨迹）导出 → fetch_all_pages → code_execute 生成 Excel\n"
    "- 多维度对比/趋势分析 → local_db_export 拿数据 → code_execute 计算+生成 Excel\n\n"
    "## 主动追问场景（必须用 ask_user）\n"
    "- 用户查'销量/销售额/付款订单'等统计需求，未说明是否排除异常订单\n"
    "  → 正常查询后，如结果中 is_exception 数量 > 0，主动告知异常订单分布并询问是否排除\n"
    "- 用户给的时间范围模糊（'最近''上次''前几天'）\n"
    "  → ask_user 给出 2-3 个具体时间选项让用户确认\n"
    "- 用户要执行写操作（取消订单、修改价格、标记异常等）\n"
    "  → ask_user 确认操作对象和影响范围，说明不可逆后果\n\n"
    "## 规则\n"
