-- 230.11: Keep legacy personal actions out of the strict Runtime owner claim.
-- Runtime v3 dispatch requires an organization-scoped action.  The legacy
-- claim function also serves older personal actions, so the final Runtime
-- wrapper marks its transaction and narrows only that claim path.
    ...
    WITH candidates AS (
        SELECT action.id
         ...
         WHERE action.status = 'queued'
           ...
           AND (
               NULLIF(current_setting('app.agent_runtime_claim_scope', TRUE), '')
                   IS DISTINCT FROM 'tenant'
               OR action.org_id IS NOT NULL
           )
           AND NOT EXISTS (
