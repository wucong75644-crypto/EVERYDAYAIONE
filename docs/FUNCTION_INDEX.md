...
| `ScheduledWecomSmartDispatchService.dispatch_claimed` / `scheduled_wecom_smart_identity` / `scheduled_wecom_receipt_hash` | `backend/services/agent/runtime/application/scheduled_wecom_smart_dispatch.py`、`scheduled_wecom_receipt.py` | Runtime-owned one-shot Smart Robot orchestration over an already claimed and verified typed payload. It rejects App or fence drift before persistence, permits transport only for fresh prepare/start owners, deduplicates same-process attempt/provider identity, sends exactly `markdown/{content}` once, maps typed ACK/rejection to SQL-parity allowlisted evidence, and conservatively records UNKNOWN for every non-definitive post-start result. Cancellation best-effort persists UNKNOWN then re-raises; failed persistence leaves `dispatch_started` for 227_48. It does not claim/read/terminalize, compose recovery, or activate a worker. |
