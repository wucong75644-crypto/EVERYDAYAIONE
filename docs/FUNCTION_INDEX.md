...
| `PostgresModelGatewayRepository` | `backend/services/agent/runtime/infrastructure/postgres/model_gateway.py` | BG2 scoped DB adapter；Runtime 仅 submit/read，Gateway 才能 claim、dispatch、renew、finalize 与 recover |
| `submit_agent_runtime_model_gateway_operation` / `read_agent_runtime_model_gateway_operation` | `backend/migrations/227_18_agent_runtime_model_gateway.sql` | Runtime 的 secret-free operation 提交/幂等 readback；read projection 不返回 encrypted bundle 或 SecretReference |
| `claim_agent_runtime_model_gateway_operation` / `mark_agent_runtime_model_gateway_dispatched` / `renew_agent_runtime_model_gateway_operation` / `finalize_agent_runtime_model_gateway_operation` / `recover_agent_runtime_model_gateway_operations` | `backend/migrations/227_18_agent_runtime_model_gateway.sql` | Gateway-only 租约、CAS、token/hash/revision/kill-epoch fencing 与 UNKNOWN 保守恢复；claim 是 encrypted configuration bundle 唯一出口，finalize 不终结 Runtime ModelAttempt |
