...
- Runtime 只验证工具调用和结构化产物，不从最终自然语言反向推断业务语义。
...
1. Tool Call 参数必须通过既有 Schema、权限和安全检查；
2. 工具执行结果区分 success、empty、partial、error 和 timeout；
3. `AgentResult`、`emit_table` 等结构化产物校验行列、状态和来源后登记 Ledger；
4. 可恢复错误通过原 Tool Observation 返回模型，由模型修正参数、重试或换工具；
5. 模型无 Tool Call 时按原链路直接完成，不缓冲、解析或改写最终自然语言。

ArtifactLedger 继续提供来源、状态、完成判断、跨 Turn 投影和审计，但不负责理解模型
最终文本。ERP、沙盒、上下文和模型最终表达的消费方式保持不变。
...
| 工具边界校验 | Tool Call + AgentResult | 原 Tool Observation + Ledger | 只校验确定性参数、状态和结构 |
| 最终回答 | model stream | 原 ContentPart / 渠道投递 | 不缓冲、不解析、不改写 |
...
- 删除最终答案 Claim 提取、证据匹配和 PASS/RETRY/BLOCK。
- 工具参数、执行状态和结构化 ToolOutput 继续使用既有确定性校验。
- 工具错误继续通过原 Observation 返回模型。
