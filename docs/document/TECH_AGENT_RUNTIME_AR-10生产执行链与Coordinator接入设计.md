...
| Tool Call → Action | `complete_model_step_and_create_actions` RPC | 同一事务解析已规范化 tool-call draft 并创建全部 Action |
...
- RPC：`prepare_model_attempt`、`record_model_attempt_unknown`、
  `complete_model_attempt_and_step`、`fail_model_attempt_and_step`、
...
5. 只有 `failed-before-dispatch` 或 provider 明确保证相同 idempotency key exactly-once/
   ...
   递增。
6. credits 只由唯一 completed attempt 的持久 usage receipt 结算；unknown 不先扣费，
   后续 provider 对账可形成独立 adjustment，禁止对同一 model_step 重复结算。

### 5.3 原子边界与竞态

- prepare attempt：Session → Run → ModelStep 锁，验证 Run token/lease/version 后插入。
- terminal：同锁序再锁 Attempt；校验 token、attempt 状态、request hash；一次性写
  attempt + ModelStep + usage event/outbox。
- cancel 与 provider completion 竞争：两者按 Session → Run 加锁；先提交者决定。
  completion 若见 Run cancelled 返回 `terminal_conflict`，只保留 provider receipt 供
  对账，不发布用户终态。
...
以新 RPC `complete_model_step_and_create_actions` 取代 Coordinator 对现有
`complete_model_step` 的调用。它按 Session → Run → ModelStep 加锁，验证 Run fencing/
lease/version、完成 ModelStep、批量插入所有 Actions、计算 `blocking_action_count`，
再追加 `model_step.completed` 和 `action.requested` events/outbox。任何一项失败全部回滚。

不允许“先完成 ModelStep，随后在 Python 创建 Action”，否则 crash 会永久丢 tool call。
...
1. **普通文本最终回答**：ingress submit command(K=request id) → C create/claim Run
   (token) → DB prepare ModelAttempt → IO Model → DB complete attempt+step →
   Completion Gate → DB complete Run(E run.completed) → web projection 写 message/task →
   WebSocket。失败从 attempt/Run/outbox 恢复。
2. **多轮模型+同步只读工具**：ModelStep complete 与 Action 创建同事务 →
...
5. **异步图片/视频 accepted**：dispatch 前 attempt；IO 返回 external receipt →
   ...
   禁止 accepted 普通重试。
6. **Provider unknown**：dispatching attempt + IO → unknown DB 记录；Run paused；
   Reconciler claim(token) readback IO；明确完成则 terminal，明确未接收且 retry-safe 才重调，
   否则人工决策。不得直接 fail+retry。
7. **用户取消与模型完成竞态**：ingress cancel command；cancel_run 与 model terminal RPC
   同锁序。先提交者胜；取消胜则本地 cancel，迟到 receipt 仅对账；完成胜则 cancel 返回
   terminal_conflict，projection 只发布一个终态。
...
9. **Actor/Coordinator 进程重启**：DB queued/expired scan 为事实源；Redis 仅唤醒；
   expired Run/attempt 被 claim/reconcile；内存 stream 丢失但 durable event 可 replay。
...
1. 先部署 additive schema/RPC：217 ModelAttempt，218 Action，219 Coordinator queue/
   projection compatibility（编号见任务表）；不改 212～216。
...
4. 门禁：真实 PostgreSQL 并发/崩溃测试通过；projection replay 对账通过；unknown 和
   accepted reconcile 演练通过；角色权限通过；旧队列可观测为零；回滚脚本和开关演练。
...
在 217/218 的 Model unknown、Action 原子创建/accepted reconcile 未完成并通过真实
...
|---|---|---|---|---|---|---|
| AR-11 | ModelAttempt unknown 持久闭合；Owner=Runtime Coordinator/Reconciler | AR-10 | `runtime/domain/model*`, `ports/model*`, `infrastructure/postgres/model*`, migrations/tests | production ingress、旧 Actor、Action | 217 独占 | 真实 PG：dispatch crash、unknown readback、cancel race、计费唯一；可与 AR-12 并行 |
| AR-12 | Action/Attempt/Result + ModelStep→Actions 同事务；Owner=Action subsystem | AR-10 | `domain/action.py`, `ports/executor.py`, 新 postgres action files、migrations/tests | Coordinator、projection、旧 handler | 218 独占 | 真实 PG：批量原子、并行 blocker、accepted/unknown；可与 AR-11 并行 |
| AR-13 | Runtime Coordinator application service、Run scanner/lease/continuation | AR-11+12 | 新 `runtime/application/`, composition tests | ingress、旧 Actor、projection projector | 无 | contract + crash/lease tests；不得与 AR-11/12 并行实施共享 ports 后再合并 |
| AR-14 | Web/WeCom Runtime ingress adapter，只建 command/run | AR-13 | 新 ingress adapter、最小 route/wecom enqueue 接线、定向测试 | Actor executor、projection、delivery sender | 无 | duplicate/cancel/scope；Web 与 WeCom 子任务不可同时改共享 composition |
| AR-15 | Runtime Projection worker + 旧事实兼容 projection | AR-12+13 | `runtime/application/projection*`, `infrastructure/projection*`, message/task/artifact compatibility tests | ingress、Action/Model schema、delivery sender | 219 独占 | 真实 PG replay、断线、outbox crash；可与 AR-14 在 AR-13 后并行 |
...
| AR-17 | 生产 wiring、single-owner feature gate、维护窗口/回滚脚本 | AR-14+15+16 | composition/startup/deploy scripts、运行手册、集成测试 | 212～219、业务工具 | 无 | 全链真实 PG + staging 演练；必须串行 |
| AR-18 | 观察期后 contract，删除旧 Chat Owner 接线 | AR-17+观察验收 | 旧 Actor Chat wiring、兼容投影清理、共享索引 | Runtime schema 历史事实 | 220 仅在确需 contract DB 变更时独占 | 不与 AR-17 并行；需单独确认 |

### 10.1 合并顺序与并行度

- 下一阶段同时启动 **2 个任务：AR-11 与 AR-12**。它们拥有不同 migration 和文件；
  若都需改 `ports/__init__.py`、`domain/__init__.py`，由总控在集成提交统一更新，子任务禁止改。
- 合并：AR-11 → AR-12 → 总控共享 export/index 集成 → AR-13。
...
- 218：Action 三表、ModelStep+Action 原子 RPC、Action claim/reconcile/wake，仅 AR-12。
- 219：projection compatibility checkpoint/mapping（审计实现时确需才建），仅 AR-15；
  不得把 Coordinator queue 混入 Action 事务。
- 220：观察期后 contract 预留，仅 AR-18；没有实际 DB contract 时不得创建空 migration。
...
- `TECH_AGENT_RUNTIME数据库RPC与原子边界.md`：登记 213～216 实际签名；说明
  `complete_model_step` 不创建 Action。
...
- `CURRENT_ISSUES.md`：记录 production Owner 仍为 Conversation Actor、216 未部署、
  Model unknown 与 Action persistence 是切换阻塞项。

## 13. 未决风险（不改变冻结 Owner）

1. 各 Provider 是否提供可按 request id 查询的状态与幂等保证，需要 AR-11 按 provider
   逐项形成 capability matrix；缺失者默认不可自动重试。
2. 旧 credits terminal RPC 的精确账务字段与 Runtime usage receipt 映射须由 AR-15
   真实 PostgreSQL 对账测试冻结，财务账本不迁移为 event。
