...
assert all(p.startswith(("backend/", "docs/document/")) for p in tracked + untracked)
...
preserved = [
    ...
    "backend/services/agent/erp_agent.py", "backend/services/agent/file_tool_mixin.py",
    "backend/services/agent/file_delete_mixin.py", "backend/services/agent/sandbox_tool_mixin.py",
...
for prefix in ("backend/config", "backend/services/kuaimai", "backend/services/erp", "backend/services/media",
               "backend/services/sandbox", "backend/migrations", "frontend"):
...
print("No new persistence payload or schema. WS approved is validated as literal bool; fields/builders/frontend unchanged.")
