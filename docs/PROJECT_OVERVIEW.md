...
- **2026-08-15**：生产 Chat Owner 回退到 Conversation Actor 与旧 AgentLoop
  - 恢复原提示词、工具注册、多轮和同轮多工具循环；Runtime 基础设施仅保留到
    `227_12`，不再接管 Web、企微或定时任务入口
  - 恢复旧 AgentLoop/ChatTool 对非 SAFE 工具的 V3 确认与执行所有权；确认协议解除
    Runtime Action/Interaction 绑定，WebSocket 仍保持脱敏确认和失败关闭
  - 一次性维护工具精确清理本次 Runtime 测试事实和 3 个独立会话，恢复 18 积分，
