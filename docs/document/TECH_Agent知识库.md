...
| 新增迁移脚本 | migrations/ | 新建 `add_knowledge_base.sql` |
...
| `backend/migrations/add_knowledge_base.sql` | ~80 | 数据库迁移脚本（3 张表 + 索引） |
...
- [ ] 任务 1.1：创建迁移脚本 `add_knowledge_base.sql`（3 张表 + 索引 + 约束）
