-- 248: 交付事件的 RPC 重试必须复用稳定 event_id，不能重复占用 delivery_seq。
--
-- PostgreSQL 仍是页面交付事实源。这里只重试事件持久化；不触发模型、工具或计费重试。
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_conversation_delivery_events_stream_event
...
DECLARE
    ...
    v_inserted BOOLEAN;
BEGIN
    IF p_task_id IS NULL OR p_execution_token IS NULL OR p_event_id IS NULL
...
REVOKE ALL ON FUNCTION public.append_conversation_delivery_event(
