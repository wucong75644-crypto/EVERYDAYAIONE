...
preserved = [
    ...
    "backend/services/tools/dispatcher.py",
...
# Approved scope repair enriches only in-process rejection/exception metadata;
# all projections and the existing raw result wrapper remain identical.
unchanged_methods("backend/services/tools/result.py", "ToolResult", ("from_exception", "not_executed"))
...
print("No new persistence payload or DB schema; scope binding and in-process resource access metadata are approved; old parameter names remain supported. WS approved is validated as literal bool; fields/builders/frontend unchanged.")
