...
- 生产 `messages.content` 为 TEXT，而统一准备 payload 的 `content` 为 JSONB；迁移 148 的 retry/reset
  更新表达式必须在 `COALESCE` 前执行显式 `::TEXT`，不能依赖赋值阶段的隐式转换。
- 已部署环境通过迁移 149 以相同签名 `CREATE OR REPLACE` 更新内部消息准备函数；新环境的迁移 148
  同步使用正确表达式。该修复不改变表结构、数据或调用协议，采用 roll-forward，不回退到已知错误函数。
- 前端仅对明确收到的 HTTP 500 回滚乐观 streaming；网络中断、超时和网关 502/503/504 的提交结果
  仍可能未知，继续保留幂等重试/查询语义，避免重复生成外部副作用。
