...
BEGIN
    ...
    ELSIF _agent_runtime_media_action_only_run_v1(v_event.run_id) THEN
        ...
        IF v_run.id IS NULL OR v_task.id IS NULL
           ...
           OR v_task.delivery_context @> '{"runtime":true}'::JSONB IS NOT TRUE THEN
            RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_RETRY_TASK_SCOPE_INVALID'
                USING ERRCODE='42501';
        END IF;
        v_facts:=jsonb_build_object('run',to_jsonb(v_run),'task',to_jsonb(v_task));
    END IF;
