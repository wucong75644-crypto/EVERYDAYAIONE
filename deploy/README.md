...
- 若当前任务没有新改动但需要再次测试同一提交，使用 `./deploy/release.sh --deploy-task <commit-sha>`；它重新部署已推送的当前任务提交。若首次发布在迁移前中断，则必须显式指定该提交内的迁移，例如 `./deploy/release.sh --deploy-task <commit-sha> --migration-file backend/migrations/242_name.sql`；入口会核验该迁移确实存在于目标提交中。
