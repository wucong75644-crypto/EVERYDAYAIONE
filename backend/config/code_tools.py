...
_DESCRIPTION_BASE = (
    ...
    "DuckDB SQL 方言：取日期 CAST(ts AS DATE) 或 ts::DATE（无 DATE() 函数）；"
    "日期截断 DATE_TRUNC('month', ts)；日期加减 ts + INTERVAL '1 day'；"
    "字符串拼接 || 不是 +；"
    "格式化日期 strftime(ts, '%m.%d') / strftime(ts, '%Y-%m-%d')；"
    "提取部分 YEAR(ts) / MONTH(ts) / DAY(ts) / HOUR(ts) / MINUTE(ts)。\n"
    "导出 Excel: df.to_excel(path, index=False, engine='xlsxwriter') "
    "自动处理 NaN/Timestamp，不要手撸 workbook.add_worksheet + worksheet.write 循环。\n"
...
_DESCRIPTION_WORKSPACE = (
    ...
    "DuckDB SQL 方言（与 MySQL/PG 差异）：\n"
    "  - 取日期: CAST(ts AS DATE) 或 ts::DATE（无 DATE() 标量函数）\n"
    "  - 日期截断: DATE_TRUNC('month', ts)\n"
    "  - 日期加减: ts + INTERVAL '1 day'\n"
    "  - 字符串拼接: || 不是 +\n"
    "  - 类型名: TIMESTAMP/BIGINT/DOUBLE/VARCHAR（无 DATETIME/INT 类型名）\n"
    "  - 格式化日期: strftime(ts, '%m.%d') / strftime(ts, '%Y-%m-%d %H:%M:%S')\n"
    "  - 提取部分: YEAR(ts) / MONTH(ts) / DAY(ts) / HOUR(ts) / MINUTE(ts) / EXTRACT(year FROM ts)\n"
    "  - 字符串↔时间: strptime('2024-01-01', '%Y-%m-%d') / ts::VARCHAR\n"
    ...
    "多 Sheet：file_analyze 已合并到同一 Parquet，用 `_sheet` 列区分来源；被跳过的 sheet 需 pd.read_excel 单独读。\n"
    "PDF 用 pdfplumber，DOCX 用 python-docx。\n"
    "图表用 ECharts JSON（.echart.json）。\n"
    "导出 Excel: df.to_excel(OUTPUT_DIR + '/文件.xlsx', index=False, engine='xlsxwriter') "
    "自动处理 NaN/Timestamp，不要手撸 workbook.add_worksheet + worksheet.write 循环。\n"
