 ...
 */
type LegacyAssetZipRequest = {
  urls: string[];
  filenames?: string[];
  zip_name?: string;
};

export function downloadUserAssetsZip(uid: string, assetIds: string[]): Promise<void>;
export function downloadUserAssetsZip(uid: string, body: LegacyAssetZipRequest): Promise<void>;
export async function downloadUserAssetsZip(
  uid: string,
  assetIdsOrLegacyBody: string[] | LegacyAssetZipRequest,
): Promise<void> {
  const token = localStorage.getItem('access_token');
  const orgId = localStorage.getItem('current_org_id');
  const byAssetId = Array.isArray(assetIdsOrLegacyBody);

  const resp = await fetch(
    byAssetId
      ? `${API_BASE_URL}/admin/users/${uid}/assets/download-zip`
      : `${API_BASE_URL}/admin/users/${uid}/download_zip`,
    {
    method: 'POST',
    headers: {
      ...
      body: JSON.stringify(
        byAssetId
          ? { asset_ids: assetIdsOrLegacyBody }
          : assetIdsOrLegacyBody,
      ),
    },
  );
