...
> 2026-07-24 状态说明：本文的跨用户 `merge_users` 自动合并设计已由迁移 155
> 安全方案取代。企微身份属于另一用户时必须返回 `MERGE_REVIEW_REQUIRED`，等待企业
> 治理域的管理员审核能力；禁止由 OAuth runtime 跨表迁移或删除用户。其余产品背景仅作
> 历史参考，当前实现以 `TECH_Web认证数据库能力门面.md` 为准。
