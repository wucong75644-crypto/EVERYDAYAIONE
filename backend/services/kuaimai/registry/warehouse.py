...
WAREHOUSE_REGISTRY = {
    ...
    "allocate_list": ApiEntry(
        ...
        param_docs={
            "status": "调拨单状态。可选值: REATED(待审核), OUTING(出库中), AUDITED(已审核), ALLOCATE(调拨中), ALLOCATE_IN(调入中), FINISHED(已完成), CANCELED(作废), MIXED(待审核和出库中且已拣选完成)。示例: ALLOCATE",
            "code": "调拨单号。示例: DB20260315001",
            "start_date": "起始日期。格式: YYYY-MM-DD。示例: 2026-03-01",
            "end_date": "结束日期。格式: YYYY-MM-DD。示例: 2026-03-15",
            "label_name": "标签名称。示例: 紧急",
        },
    ...
    "allocate_in_list": ApiEntry(
        ...
        param_docs={
            "status": "调拨入库单状态。可选值: CREATED(已创建), ALLOCATE_OUT(收货完成), SHELVED(已上架), FINISHED(已完成), CANCELED(作废)。示例: CREATED",
    ...
    "allocate_out_list": ApiEntry(
        ...
        param_docs={
            "status": "调拨出库单状态。可选值: CREATED(未出库), OUTING(未出库), FINISHED(已出库), CANCELED(已完成/作废)。示例: CREATED",
            "code": "调拨出库单号。示例: DB20260315001",
            "time_type": "时间类型。可选值: create(创建时间), out(出库时间), gm_modified(修改时间,默认)。示例: gm_modified",
            "start_date": "起始日期。格式: YYYY-MM-DD。示例: 2026-03-01",
            "end_date": "结束日期。格式: YYYY-MM-DD。示例: 2026-03-15",
        },
        formatter="format_other_in_out_list",
    ...
    "other_in_list": ApiEntry(
        ...
        param_docs={
            "status": "入库单状态。可选值: NOT_FINISH(未完成), FINISHED(已完成), CLOSED(已关闭)。示例: NOT_FINISH",
    ...
    "other_out_list": ApiEntry(
        ...
        param_docs={
            "status": "出库单状态。可选值: 0(已作废), 1(待出库), 3(已出库), 4(出库中), 5(草稿)。示例: 1",
    ...
    "inventory_sheet_list": ApiEntry(
        ...
        param_docs={
            "status": "盘点单状态。可选值: 1(草稿), 2(盘点中), 3(盘点完成), 4(已作废)。多个逗号拼接。示例: 2",
    ...
    "process_order_list": ApiEntry(
        ...
        param_docs={
            "status": "加工单状态。可选值: WAIT_VERIFY(待审核), WAIT_PRODUCT(待加工), PRODUCING(加工中), FINISHED(已完成), CLOSED(已作废)。示例: WAIT_PRODUCT",
