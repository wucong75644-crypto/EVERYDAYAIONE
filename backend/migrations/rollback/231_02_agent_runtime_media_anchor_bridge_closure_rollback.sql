-- Restore the pre-231.02 direct media anchor bridge.
...
BEGIN
    ...
    IF definition IS NULL OR position(old_fragment IN definition) = 0 THEN
        RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_CHAT_INPUT_BRIDGE_ROLLBACK_DRIFT'
            USING ERRCODE = '55000';
    END IF;
...
DECLARE
    ...
    old_fragment CONSTANT TEXT := $old$
        ...
        p_output_message_id::TEXT,p_task_id::TEXT,1,p_tool_name,safe_arguments,$old$;
    new_fragment CONSTANT TEXT := $new$
        p_conversation_id,p_org_id,p_user_id,p_task_id::TEXT,
        p_input_message_id::TEXT,p_task_id::TEXT,1,p_tool_name,safe_arguments,$new$;
BEGIN
    SELECT pg_get_functiondef(target) INTO definition;
    IF definition IS NULL OR position(old_fragment IN definition) = 0 THEN
        RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_OUTPUT_BRIDGE_ROLLBACK_DRIFT'
            USING ERRCODE = '55000';
    END IF;
    EXECUTE replace(definition, old_fragment, new_fragment);
END
