...
_DESCRIPTION_WORKSPACE = (
    ...
    "工作目录(cwd)就是用户文件目录。\n"
    "STAGING_DIR 存 ERP 查询结果和 file_analyze 的 Parquet 缓存。\n"
    "OUTPUT_DIR 存输出文件，自动上传。\n"
    "数据文件（Excel/CSV）不要用 openpyxl 读取，先调 file_analyze 获取 Parquet 路径，\n"
    "再用 duckdb.sql() 查询。PDF 用 pdfplumber，DOCX 用 python-docx。\n"
