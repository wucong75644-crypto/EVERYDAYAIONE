...
export REDIS_PORT=1
export TOOL_RESULT_PAYLOAD_WRITE_VERSION=1
export PYTHONPATH=/private/tmp/tool05-testdeps:backend
export MPLCONFIGDIR=/private/tmp/tool07-matplotlib
export IPYTHONDIR=/private/tmp/tool07-ipython
"$tool_test_python" -m pytest \
 backend/tests/test_tool_definitions_07.py backend/tests/test_tool_definitions_07_entries.py \
 backend/tests/test_planner_framework.py backend/tests/test_scheduled_task_changeset_adapter.py \
 backend/tests/test_tool_args_validator.py backend/tests/test_erp_local.py \
 -o addopts='' -v --tb=short > docs/document/tool-unification-evidence/07-contracts.txt 2>&1
"$tool_test_python" -m pytest \
 ...
 -o addopts='' -v --tb=short > docs/document/tool-unification-evidence/07-integration.txt 2>&1
"$tool_test_python" -m pytest \
 ...
 -o addopts='' -v --tb=short > docs/document/tool-unification-evidence/07-regression.txt 2>&1
"$tool_test_python" -m pytest backend/tests/test_erp_tool_mixin_unit.py \
 -o addopts='' -v --tb=short > docs/document/tool-unification-evidence/07-erp.txt 2>&1
