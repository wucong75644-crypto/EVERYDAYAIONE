-- Preserve configured Skills and history: rollback only before any binding is saved.
DO $$ BEGIN
    LOCK TABLE public.scheduled_tasks IN ACCESS EXCLUSIVE MODE;
    IF EXISTS (SELECT 1 FROM scheduled_tasks WHERE status='running') OR EXISTS (
        SELECT 1 FROM scheduled_task_config_revisions
        WHERE definition_snapshot#>'{skill_revision_snapshot,skills}' <> '[]'::jsonb
    ) THEN RAISE EXCEPTION 'SCHEDULED_SKILL_ROLLBACK_REQUIRES_EMPTY_HISTORY'; END IF;
END $$;
DROP TRIGGER scheduled_run_snapshot_immutable ON public.scheduled_task_runs;
DROP TRIGGER zz_scheduled_task_config ON public.scheduled_tasks;
DROP FUNCTION public.guard_scheduled_run_snapshot(),public.capture_scheduled_task_config();
ALTER TABLE public.scheduled_task_runs DROP COLUMN config_revision, DROP COLUMN skill_revision_snapshot, DROP COLUMN definition_snapshot;
ALTER TABLE public.scheduled_tasks DROP COLUMN config_revision, DROP COLUMN run_config_revision,
 DROP COLUMN retry_config_revision, DROP COLUMN skill_revision_snapshot;
DROP FUNCTION public.scheduled_task_definition(public.scheduled_tasks);
DROP TABLE public.scheduled_task_config_revisions;
DROP FUNCTION public.guard_scheduled_config_history();
DROP TRIGGER skill_revision_review_attestation ON public.skill_revisions;
DROP FUNCTION public.attest_skill_revision_review();
ALTER TABLE public.skill_revisions DROP COLUMN reviewed;
