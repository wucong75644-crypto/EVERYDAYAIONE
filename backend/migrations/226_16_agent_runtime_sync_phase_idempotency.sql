-- 226_16: same-phase readback is idempotent; different facts conflict.
SET LOCAL ROLE everydayai_owner;
CREATE OR REPLACE FUNCTION record_agent_sync_phase_v3(
...
DECLARE a agent_action_attempts%ROWTYPE; latest TEXT; prior agent_action_sync_phase_facts%ROWTYPE;
BEGIN
  ...
  END IF;
  SELECT * INTO prior FROM agent_action_sync_phase_facts
    WHERE action_id=a.action_id AND attempt_id=a.id AND phase=p_phase FOR UPDATE;
  IF FOUND THEN
    IF prior.request_hash IS DISTINCT FROM p_request_hash
       OR prior.checkpoint IS DISTINCT FROM COALESCE(p_checkpoint,'{}'::JSONB)
       OR prior.provider_receipt IS DISTINCT FROM COALESCE(p_provider_receipt,'{}'::JSONB) THEN
      RAISE EXCEPTION 'AGENT_SYNC_PHASE_IDEMPOTENCY_CONFLICT' USING ERRCODE='23505';
    END IF;
    RETURN jsonb_build_object('outcome','recorded','phase',p_phase,'idempotent',TRUE);
  END IF;
  SELECT phase INTO latest FROM agent_action_sync_phase_facts
    WHERE action_id=a.action_id AND attempt_id=a.id ORDER BY created_at DESC LIMIT 1;
