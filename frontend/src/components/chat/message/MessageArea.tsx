...
function ScrollToBottomButton() {
  ...
  return (
    <button
      ...
      className={`absolute bottom-6 left-1/2 transform -translate-x-1/2 px-4 py-2 rounded-full shadow-lg flex items-center justify-center transition-base z-20 ${
        hasNewMessages
          ? 'bg-accent text-text-on-accent hover:bg-accent-hover'
          : 'bg-surface-card text-text-secondary hover:bg-hover border border-border-default'
      }`}
    >
      {hasNewMessages ? (
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-surface-card">
        <div className="text-center max-w-md px-4">
          ...
          <h2 className="text-lg font-medium text-text-primary mb-2">暂无消息</h2>
          <p className="text-text-tertiary text-sm">在下方输入框中输入内容开始对话</p>
        </div>
      </div>
    );
  }
