/* 230.09: the closed production gate must not block its own readiness probe. */
...
BEGIN
    ...
    IF media_control.singleton IS NULL THEN
        RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_READINESS_MISSING'
            USING ERRCODE='55000';
    END IF;
    /* The audited activation RPC remains the only production gate opener. */
    effective_ready:= (
        p_ready
