...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM agent_action_results) OR EXISTS (SELECT 1 FROM agent_action_cost_settlements WHERE kind IN ('settle','refund','release')) THEN
    RAISE EXCEPTION 'ROLLBACK_GUARD_AGENT_TERMINAL_FACTS';
  END IF;
END $$;
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE NOT EXISTS (
            ...
            WHERE artifact.id = artifact_id::UUID AND artifact.conversation_id = p_conversation_id
    ...
    v_hash := encode(sha256(convert_to(jsonb_build_object(
        ...
        'external_receipt', COALESCE(p_result->'external_receipt', '{}'::JSONB), 'error_code', p_result->>'error_code'
    )::TEXT, 'UTF8')), 'hex');
    RETURN v_hash;
END;
$$;
