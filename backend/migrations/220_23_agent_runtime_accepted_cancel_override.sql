-- 220_23: Preserve accepted/unknown Actions for provider reconciliation.
...
DECLARE
    ...
    v_event JSONB; v_session_id UUID; v_reconciliation_count INTEGER;
BEGIN
    ...
    END LOOP;
    SELECT count(*) INTO v_reconciliation_count FROM agent_actions
     WHERE run_id = p_run_id AND status IN ('accepted', 'unknown');
    ...
    v_event := append_agent_runtime_event(
        ...
        session_user, jsonb_build_object(
            'reason', p_reason,
            'pending_reconciliation_count', v_reconciliation_count),
        ARRAY['web_runtime', 'audit']::TEXT[]);
    RETURN jsonb_build_object(
