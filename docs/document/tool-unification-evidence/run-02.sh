#!/usr/bin/env bash
# Block 02: pure new layer, then related legacy regressions; no real services.
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
"$tool_test_python" -m pytest \
    ...
    -o addopts='' -v --tb=short > docs/document/tool-unification-evidence/02-isolated.txt 2>&1
"$tool_test_python" -m pytest \
    ...
    -o addopts='' -v --tb=short > docs/document/tool-unification-evidence/02-regression.txt 2>&1
...
"$tool_test_python" -m pytest backend/tests/test_erp_tool_mixin_unit.py \
    -o addopts='' -v --tb=short > docs/document/tool-unification-evidence/02-regression-erp.txt 2>&1
