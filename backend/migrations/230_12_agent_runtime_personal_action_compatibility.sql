-- 230.12: Preserve personal-session Actions in the Runtime v3 owner path.
-- Personal sessions intentionally have org_id = NULL.  Tenant kill fences
-- apply only to organization-scoped Actions; CAS, request-hash and execution
-- token checks still apply to both scopes.
...
BEGIN
    ...
    END IF;
    IF x.org_id IS NULL THEN
        RETURN jsonb_build_object(
            'outcome','allowed','tenant_kill_epoch',0,
...
BEGIN
    ...
    SELECT * INTO x FROM agent_actions WHERE id=a.action_id;
    IF a.id IS NULL OR x.id IS NULL THEN
