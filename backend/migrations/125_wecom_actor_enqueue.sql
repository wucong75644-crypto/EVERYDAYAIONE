-- 125: 企业微信消息、占位消息与 Actor task 的原子幂等入队
-- 依赖 121 enqueue_generation_turn。
...
BEGIN
    IF p_task_data IS NULL
       ...
       OR NOT (p_delivery_context @> '{"actor": true, "channel": "wecom"}'::JSONB)
       OR COALESCE(p_delivery_context->>'chatid', '') = ''
       OR COALESCE(p_delivery_context->>'transport', '') NOT IN ('smart_robot', 'app') THEN
        RAISE EXCEPTION 'WECOM_ACTOR_ENQUEUE_ARGUMENT_INVALID' USING ERRCODE = '22023';
    END IF;
    ...
    EXCEPTION WHEN invalid_text_representation THEN
        RAISE EXCEPTION 'WECOM_ACTOR_ENQUEUE_ID_INVALID' USING ERRCODE = '22023';
    END;
    ...
    SELECT * INTO v_conversation
     ...
     FOR UPDATE;
    IF NOT FOUND
       OR v_conversation.user_id IS DISTINCT FROM (p_task_data->>'user_id')::UUID
    ...
    IF v_input.id IS NULL
       ...
       OR v_input.content IS DISTINCT FROM p_input_content THEN
        RAISE EXCEPTION 'WECOM_ACTOR_MESSAGE_CONFLICT' USING ERRCODE = '23505';
    END IF;
