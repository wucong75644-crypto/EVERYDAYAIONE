...
ERP_ROUTING_PROMPT = (
    ...
    "## 高频易混淆场景\n"
    "### 库存查询\n"
    "- 商品库存 → stock_status(outer_id=XX)\n"
    "- 规格/SKU库存 → stock_status(sku_outer_id=XX)\n"
    "- 各仓库分布 → warehouse_stock\n"
    "- 进出流水 → stock_in_out\n"
    "- ⚠ outer_id/sku_outer_id 区分详见参数文档中的 ⚠ 提示。"
    "系统会自动纠正高置信度的编码类型错误，零结果时会自动建议替代参数\n\n"
    "### 售后查询（跨3个工具）\n"
    ...
    "### 出库查询（跨3个工具）\n"
    "- 销售出库 → erp_trade_query(outstock_query)\n"
    "- 出库单详情 → erp_trade_query(outstock_order_query)\n"
    "- 商品出入库流水 → erp_product_query(stock_in_out)\n"
    "- 非销售出入库 → erp_warehouse_query(other_out_list/other_in_list)\n\n"
    "### 标签查询（同名action不同工具）\n"
