...
| 入口 | 判定 | 实际 Owner |
|---|---|---|
| Web Runtime ingress | `RuntimeIngress` 调用 `runtime_submit_ingress_v4`，兼容回退仅在 RPC 不存在时使用 v3/v2 | Runtime command claim → ModelLoop/ActionLoop |
| WeCom ingress | `services/wecom/actor_enqueue.py` 在 `agent_runtime_ingress_enabled` 下调用 `enqueue_wecom_runtime_turn_v6`，关闭时仍调用历史 generation RPC | Runtime 或 legacy generation，按显式开关分流 |
