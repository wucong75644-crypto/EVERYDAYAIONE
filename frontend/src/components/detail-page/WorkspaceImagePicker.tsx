...
function PickerImage({ item }: { item: PickerItem }) {
  ...
  const src = !proxyFailed ? (thumbnail.src || proxyUrl) : '';
  ...
  return src ? (
    <img
      ...
      onError={() => {
        if (!thumbnail.failed) thumbnail.onError();
        else setProxyFailed(true);
      }}
    />
  ) : (
    <span className="w-full h-full flex items-center justify-center text-xs text-[var(--s-text-tertiary)]">图片加载失败</span>
  );
}
