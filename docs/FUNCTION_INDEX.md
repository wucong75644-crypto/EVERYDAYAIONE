...
| `build_safe_runtime_composition` / `build_runtime_data_read_registry` | `backend/services/agent/runtime/production_composition.py`、`backend/services/agent/runtime/data_read_composition.py` | 从真实 Runtime read registry 构造 safe composition；仅在显式注入时增加 local_data/file_analyze/fetch_all_pages，ERP Write/Sync、淘宝奇门、Media 不注册 |
