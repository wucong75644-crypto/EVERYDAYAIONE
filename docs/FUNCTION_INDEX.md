...
| `read_agent_runtime_scheduled_wecom_dispatch_payload_v1` | `backend/migrations/227_46_agent_runtime_scheduled_wecom_dispatch_payload.sql` | Fenced worker-only gate over the approved 227_38 live dispatch context. A completed text item reads only the same finalized `scheduled_task_runs.result_summary`, verifies exact run/task/finalization/content/item/version identity, and returns an allowlisted text payload with opaque target/content hashes plus App `corp_id`/`wecom_userid` or Smart Robot `chatid`. Artifact and non-completed items return fixed payload-free `unsupported`; the wrapper never exposes model output, target snapshots, internal mapping/user IDs, Secret, URL or artifact bytes. It inherits 227_38's pre-attempt unavailable cancellation without passing through the full context response. |
| `ScheduledWecomDeliveryRepositoryPort` / `PostgresScheduledWecomDeliveryRepository` | `backend/services/agent/runtime/ports/scheduled_wecom_delivery.py`、`infrastructure/postgres/scheduled_wecom_delivery.py` | Frozen typed boundary over claim v2, prepared recovery, 227_45 dispatch v2, 227_40 outcome and 227_41～44 reconciliation RPCs. Explicit parameters preserve request/worker/token/version/hash/revision fences; exact output allowlists reject malformed or sensitive fields, and durable mutations replay only their identical request after a lost response. |
