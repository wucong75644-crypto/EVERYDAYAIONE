...
后续状态：本记录修复已随 `eeccf3bd` 完整部署；用户再发现“执行内容包含整段创建原话”。本记录中“原文直接成为 prompt、补全时隐藏已知内容”的实现已由 [执行内容整理复验](SCHEDULED_TASK_CONTENT_EXTRACTION_ACCEPTANCE.md) 修正，原文保留在说明中，业务内容独立提取并显示。该后续修复尚未部署，不能沿用本记录的历史结论关闭验收。
