...
| `ScheduledWecomSmartDispatchService.dispatch_claimed` / `SmartRobotTransportResolverPort.resolve_smart_transport` / `scheduled_wecom_smart_identity` | `backend/services/agent/runtime/application/scheduled_wecom_smart_dispatch.py`、`ports/scheduled_wecom_smart_dispatch.py`、`scheduled_wecom_receipt.py` | Runtime-owned one-shot Smart Robot orchestration over an already claimed and verified typed payload. It rejects App or fence drift, then resolves an exact connected transport by target org inside the routed item/provider single-flight and before persistence; resolver failure returns typed UNAVAILABLE with no prepare/start/send/UNKNOWN facts, while cancellation propagates. One shielded owner then performs fresh prepare→start→send→outcome; typed ACK/rejection maps to SQL-parity allowlisted evidence and every non-definitive post-start result records UNKNOWN. Transport cancellation best-effort persists UNKNOWN then re-raises; failed persistence leaves `dispatch_started` for 227_48. |
| `ScheduledSmartTransportResolver.resolve_smart_transport` | `backend/services/wecom/scheduled_smart_transport.py` | Trusted credential-free adapter over injected `get_ws_client(org_id)`. It accepts only canonical org UUIDs and returns only an exact-org client with `is_connected is True` and callable `send_proactive_typed`; it does not read Secret, cache clients, inspect chatid, or use a mutable global default. |
