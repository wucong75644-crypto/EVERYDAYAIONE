...
> **版本**：v2.5 | **日期**：2026-02-05 | **状态**：确认实施（含断点续传 + 保底计费 + 同源说明 + tabId 增强）
...
class TabSyncManager {
  ...
  constructor() {
    // 使用 crypto.randomUUID() 防止碰撞，详见 TECH_WEBSOCKET实时推送.md 11.5.3
    this.tabId = typeof crypto !== 'undefined' && crypto.randomUUID
      ? `tab-${crypto.randomUUID()}`
      : `tab-${Date.now()}-${Math.random().toString(36).slice(2, 10)}-${Math.random().toString(36).slice(2, 10)}`;
...
# 心跳间隔（避免网关超时）
HEARTBEAT_INTERVAL = 30  # 秒
# 消息缓冲区大小（字节 + 时间双维度，详见 TECH_WEBSOCKET实时推送.md 11.5.1）
...
class StreamState:
    ...
    connection_version: int = 0
    buffer: List[Tuple[float, int, str]] = field(default_factory=list)  # (timestamp, index, msg)
...
class ChatStreamManager:
    ...
    async def subscribe(
        ...
        async with self._lock:
            ...
            # 避免网络波动重连时收到重复内容
            # buffer 结构: List[Tuple[float, int, str]] = (timestamp, index, msg)
            for _, idx, msg in state.buffer:
                if idx > last_received_index:
                    queue.put_nowait(msg)

            # 返回已累积的内容和当前缓冲区索引
            current_index = state.buffer[-1][1] if state.buffer else -1
            return queue, state.full_content, connection_id, current_index
    ...
    def _broadcast(self, task_id: str, state: StreamState, data: Any):
        ...
        if msg:
            ...
            state.buffer.append((now, msg_index, msg))
            state.buffer_bytes += msg_bytes

            # 清理超过 5 分钟的旧消息
            cutoff_time = now - BUFFER_MAX_AGE_SECONDS
            while state.buffer and state.buffer[0][0] < cutoff_time:
                _, _, old_msg = state.buffer.pop(0)
                state.buffer_bytes -= len(old_msg.encode('utf-8'))

            # 清理超过 1MB 的旧消息
            while state.buffer_bytes > MAX_BUFFER_SIZE_BYTES and state.buffer:
                _, _, old_msg = state.buffer.pop(0)
                state.buffer_bytes -= len(old_msg.encode('utf-8'))
...
| **缓冲区内存溢出（v2.5）** | 字节+时间双维度清理：1MB + 5分钟，详见 TECH_WEBSOCKET 11.5.1 | ChatStreamManager |
| **时间戳精度不足排序跳变（v2.5）** | 微秒级 `isoformat(timespec='microseconds')` + 序列号兜底 | 数据库写入 |
| **tabId 碰撞（v2.5）** | 优先 `crypto.randomUUID()`，详见 TECH_WEBSOCKET 11.5.3 | tabSync.ts |
...
| **缓冲区内存溢出（v2.5）** | 低 | 字节+时间双维度清理：1MB + 5分钟 |
| **时间戳精度排序跳变（v2.5）** | 极低 | 微秒级精度 + 序列号双排序 |
| **tabId 碰撞（v2.5）** | 极低 | crypto.randomUUID() 优先 |
