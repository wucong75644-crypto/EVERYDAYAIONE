...
后续状态：本升级已以 **8773b8b7** 完成受控生产部署。用户随后在创建流程发现表单未显示，已留在原任务修复；[2026-09-12 表单缺失复验记录](SCHEDULED_TASK_FORM_FIX_ACCEPTANCE.md) 为当前有效状态。此次修复尚未重新部署，用户验收未关闭。下面保留上次开发/发布准备快照，不将历史通过当作本次生产缺陷已被覆盖。
