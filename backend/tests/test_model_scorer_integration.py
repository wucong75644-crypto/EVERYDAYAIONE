...
# ============ BackgroundTaskWorker 意图提炼集成测试 ============


class TestRunIntentDistillation:
    """BackgroundTaskWorker._run_intent_distillation 测试"""
    ...
    @pytest.mark.asyncio
    async def test_skips_within_24_hours(self):
        """24 小时内再次调用被跳过"""
        worker = self._make_worker()
        worker._last_intent_distillation = datetime.now(timezone.utc)
    ...
    @pytest.mark.asyncio
    async def test_runs_after_24_hours(self):
        """超过 24 小时后重新执行"""
        worker = self._make_worker()
    ...
    @pytest.mark.asyncio
    async def test_exception_caught_timestamp_updated(self):
