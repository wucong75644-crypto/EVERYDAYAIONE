-- AR-18-A1.2-B1: command scanner observes task cancel intents before Run DML.
SET LOCAL ROLE everydayai_owner;

CREATE OR REPLACE FUNCTION _agent_command_run_eligibility(
...
BEGIN
    v_intent := _lock_agent_runtime_task_cancel_intent(p_command.id);
    IF v_intent.id IS NOT NULL THEN
        RETURN jsonb_build_object(
            'outcome', 'cancel_requested', 'intent_id', v_intent.id,
            'intent_status', v_intent.status, 'run_id', v_intent.run_id);
    END IF;
...
CREATE OR REPLACE FUNCTION _claim_eligible_agent_command(
...
BEGIN
    ...
    IF v_intent.id IS NOT NULL THEN
        RETURN jsonb_build_object(
            'outcome', 'cancel_requested', 'command_id', p_command.id,
            'intent_id', v_intent.id, 'run_id', v_intent.run_id);
    END IF;
...
BEGIN
    ...
    FOR v_candidate IN
        SELECT command.id, command.session_id
          ...
          LEFT JOIN agent_runtime_task_cancel_intents intent
            ON intent.submit_command_id = command.id
    ...
    LOOP
        ...
        IF v_eligibility->>'outcome' IN ('busy', 'cancel_requested') THEN
            v_command.id := NULL;
            CONTINUE;
        END IF;
        EXIT;
