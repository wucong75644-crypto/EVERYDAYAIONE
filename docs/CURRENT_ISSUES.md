...
### 2026-09-04 KIE 外部图片读取失败 — 开发完成，待部署验证

- 根因是 KIE 图生图/图生视频直接拉取 CDN 参考图；上游读取具有短超时和非确定性，生产中表现为 `Image fetch failed`，与本地任务 ID、文件夹中文路径或回调绑定无关。
- 用户上传、工作区文件夹、CDN/NAS 路径与任务生命周期保持不变。仅在 KIE 适配器即将创建外部任务前，由后端读取已上传的自有 CDN/OSS 图片并通过 KIE File Stream Upload 交付，`createTask` 只收到 KIE 临时 URL。
- 图片和视频适配器共用同一素材交付组件；同一请求内重复引用的素材只上传一次。未知第三方 URL 被拒绝，防止后端将非工作区内容转交给 KIE。
- 上传失败不产生 KIE 外部任务，继续由既有本地任务失败与退款逻辑收尾；KIE 回调、轮询、结果回存不变。部署后需验证图生图、图生视频、工作区插入素材和批量生成场景，并观察 `Image fetch failed` 是否消失。
