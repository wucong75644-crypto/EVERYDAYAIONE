-- 100: 维度 ID 补全 Phase 3 — RPC 改用 ID 分组 + JOIN 维度表取名称
--
-- 改造 erp_global_stats_query 和 erp_order_stats_grouped:
-- shop/supplier/warehouse 分组从名称字符串改为 ID 分组 + LEFT JOIN 取名称
-- 向后兼容：group_key 输出值仍是名称，Python 端无需改动
...
COMMENT ON FUNCTION erp_global_stats_query IS
    'ERP全局统计RPC — 100: 维度分组改用 ID + LEFT JOIN 取名称';
...
COMMENT ON FUNCTION erp_order_stats_grouped IS
    '订单分类统计RPC — 100: 维度分组改用 ID + LEFT JOIN 取名称';
