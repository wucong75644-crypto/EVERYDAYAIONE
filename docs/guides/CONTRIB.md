...
每个 Codex 对话使用独立的 `codex/task/*` 工作树，并从最新 `origin/main` 创建。
提交部署时，发布入口会自动完成：

```text
提交任务文件 → 推送任务分支 → 合并到 main → 部署 main → 保留工作树供测试
```

测试过程中可以反复执行提交部署；每次都生成新的 `main` 候选发布版本，并保留上一版
生产提交用于回退。候选版本不会推进稳定版本，也不会修改其他任务的开发基座。只有
用户明确说“清理工作树”后，才会确认已部署的生产候选提交（不重复部署）、标记稳定版本，并把稳定 `main` 合并
推送到其他干净的开发任务分支，然后删除当前工作树和本地任务分支。发现其他任务有
未提交修改、分支分叉或合并冲突时，流程会停止，不会自动覆盖。远程分支和 Git 历史
始终保留。
