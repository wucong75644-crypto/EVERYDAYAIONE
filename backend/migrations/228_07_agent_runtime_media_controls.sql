...
CREATE TABLE agent_runtime_media_cancel_requests (
    ...
    action_status_at_request TEXT NOT NULL,
    idempotency_key TEXT NOT NULL CHECK (length(btrim(idempotency_key)) BETWEEN 1 AND 200),
    requested_at TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp()
);
CREATE INDEX idx_agent_runtime_media_cancel_message
    ON agent_runtime_media_cancel_requests(output_message_id, requested_at, action_id);
CREATE TABLE agent_runtime_media_retry_lineage (
    ...
    base_slot_revision BIGINT NOT NULL CHECK (base_slot_revision >= 0),
    idempotency_key TEXT NOT NULL CHECK (length(btrim(idempotency_key)) BETWEEN 1 AND 200),
    created_at TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp(),
    UNIQUE (output_message_id, slot_index, retry_ordinal),
    UNIQUE (output_message_id, idempotency_key)
);
CREATE INDEX idx_agent_runtime_media_retry_slot
    ON agent_runtime_media_retry_lineage(output_message_id, slot_index, retry_ordinal DESC);
...
CREATE POLICY agent_runtime_media_cancel_owner_all
    ON agent_runtime_media_cancel_requests FOR ALL TO everydayai_owner
    USING (TRUE) WITH CHECK (TRUE);
CREATE POLICY agent_runtime_media_retry_owner_all
    ON agent_runtime_media_retry_lineage FOR ALL TO everydayai_owner
    USING (TRUE) WITH CHECK (TRUE);
...
CREATE FUNCTION _agent_runtime_media_web_control_v1(
...
BEGIN
    IF NULLIF(current_setting('app.request_id', TRUE), '') IS NULL OR NOT (
        ...
        OR (session_user = 'everydayai_wecom_runtime'
            AND current_setting('app.access_kind', TRUE) = 'runtime')
    ...
    END IF;
END $$;
CREATE FUNCTION request_agent_runtime_media_message_cancel_v1(
    p_output_message_id UUID, p_org_id UUID, p_user_id UUID,
...
BEGIN
    ...
    IF p_output_message_id IS NULL
       OR length(btrim(COALESCE(p_idempotency_key, ''))) NOT BETWEEN 1 AND 200 THEN
        RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_CANCEL_INVALID' USING ERRCODE = '22023';
    END IF;
    SELECT * INTO message FROM messages WHERE id = p_output_message_id FOR UPDATE;
    IF message.id IS NULL OR message.org_id IS DISTINCT FROM p_org_id
       OR message.conversation_id IS NULL OR message.role::TEXT <> 'assistant' THEN
        RETURN jsonb_build_object('outcome', 'not_runtime_media');
    END IF;
    PERFORM 1 FROM agent_runtime_media_action_bindings binding
     WHERE binding.output_message_id = message.id
       AND binding.org_id IS NOT DISTINCT FROM p_org_id
       AND binding.user_id = p_user_id FOR UPDATE;
    IF NOT FOUND THEN RETURN jsonb_build_object('outcome', 'not_runtime_media'); END IF;
    PERFORM 1 FROM agent_actions action
     JOIN agent_runtime_media_action_bindings binding ON binding.action_id = action.id
     WHERE binding.output_message_id = message.id ORDER BY action.id FOR UPDATE OF action;
    PERFORM 1 FROM agent_action_attempts attempt
     JOIN agent_runtime_media_action_bindings binding ON binding.action_id = attempt.action_id
     WHERE binding.output_message_id = message.id ORDER BY attempt.id FOR UPDATE OF attempt;
    PERFORM 1 FROM agent_runtime_provider_submission_facts fact
     JOIN agent_runtime_media_action_bindings binding ON binding.action_id = fact.action_id
     WHERE binding.output_message_id = message.id ORDER BY fact.id FOR UPDATE OF fact;
    -- A dispatching attempt has crossed the side-effect gate.  Preserve it as
    -- unknown so Run cancellation can only hand it to cancel/readback/reconcile.
    FOR candidate IN
    ...
    LOOP
        UPDATE agent_action_attempts SET status = 'unknown',
            ambiguity_evidence = jsonb_build_object(
                'kind', 'message_cancel_after_dispatch',
    ...
    END LOOP;
    INSERT INTO agent_runtime_media_cancel_requests(
        action_id, output_message_id, run_id, org_id, user_id, disposition,
        action_status_at_request, idempotency_key
    )
    SELECT action.id, message.id, action.run_id, p_org_id, p_user_id,
      ...
      FROM (
          SELECT DISTINCT ON (binding.action_index) action.*
            FROM agent_actions action
            JOIN agent_runtime_media_action_bindings binding ON binding.action_id = action.id
           WHERE binding.output_message_id = message.id
           ORDER BY binding.action_index, binding.created_at DESC, action.id DESC
      ) action
    ...
    FOREACH run_id IN ARRAY COALESCE(active_runs, ARRAY[]::UUID[]) LOOP
        ...
        IF run_row.status NOT IN ('completed','failed','cancelled') THEN
            ...
            IF result->>'outcome' NOT IN ('cancelled','already_cancelled') THEN
                RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_CANCEL_CONFLICT'
                    USING ERRCODE = '40001';
            END IF;
        END IF;
    END LOOP;

    SELECT count(*) FILTER (WHERE action.status = 'cancelled'),
           count(*) FILTER (WHERE action.status IN ('accepted','unknown')),
    ...
    RETURN jsonb_build_object(
        'outcome', CASE WHEN COALESCE(cardinality(active_runs), 0) = 0
                        THEN 'already_terminal' ELSE 'cancel_requested' END,
        'cancelled_count', cancelled_count,
        'reconcile_count', reconcile_count,
        'completed_count', completed_count,
        'release_task_ids', (SELECT COALESCE(jsonb_agg(DISTINCT task_id), '[]')
            FROM (SELECT binding.chat_task_id AS task_id
                  ...
                  SELECT binding.task_id
                   ...
                   WHERE binding.output_message_id=message.id
                     AND cancel_request.disposition='cancel_now'
                     AND action.status='cancelled') releasable));
END $$;
CREATE FUNCTION retry_agent_runtime_media_slot_v1(
    p_output_message_id UUID, p_conversation_id UUID, p_slot_index INTEGER,
    p_slot_id UUID, p_expected_slot_revision BIGINT, p_org_id UUID,
    p_user_id UUID, p_idempotency_key TEXT, p_client_task_id TEXT,
    p_task_slot_id TEXT DEFAULT NULL
) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog, public AS $$
DECLARE
    message messages%ROWTYPE;
    source_binding agent_runtime_media_action_bindings%ROWTYPE;
    source_action agent_actions%ROWTYPE;
    ...
    replay_lineage agent_runtime_media_retry_lineage%ROWTYPE;
    canonical JSONB;
    action_id UUID := gen_random_uuid();
    transaction_id UUID := gen_random_uuid();
    slot JSONB;
    stable_slot_id UUID;
    slot_revision BIGINT;
...
BEGIN
    ...
    IF p_output_message_id IS NULL OR p_conversation_id IS NULL OR p_slot_id IS NULL
       ...
       OR (p_task_slot_id IS NOT NULL AND length(p_task_slot_id) > 200) THEN
        RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_RETRY_INVALID' USING ERRCODE = '22023';
    END IF;
    SELECT * INTO message FROM messages WHERE id=p_output_message_id FOR UPDATE;
    IF message.id IS NULL OR message.org_id IS DISTINCT FROM p_org_id
       OR message.conversation_id IS DISTINCT FROM p_conversation_id
       OR message.role::TEXT <> 'assistant' THEN
        RETURN jsonb_build_object('outcome','not_runtime_media');
    END IF;
    SELECT runtime_session.* INTO session
      FROM agent_runtime_media_action_bindings binding
      JOIN agent_runtime_sessions runtime_session ON runtime_session.id=binding.session_id
     WHERE binding.output_message_id=message.id
       AND binding.org_id IS NOT DISTINCT FROM p_org_id
       AND binding.user_id=p_user_id
     ORDER BY binding.created_at, binding.action_id LIMIT 1
     FOR UPDATE OF runtime_session;
    IF session.id IS NULL OR session.org_id IS DISTINCT FROM p_org_id
       OR session.user_id IS DISTINCT FROM p_user_id THEN
        RETURN jsonb_build_object('outcome','not_runtime_media');
    END IF;
    SELECT * INTO replay_lineage FROM agent_runtime_media_retry_lineage
     WHERE output_message_id=message.id AND idempotency_key=btrim(p_idempotency_key);
    IF FOUND THEN
    ...
    IF EXISTS (
        ...
        WHERE binding.output_message_id=message.id
          AND binding.action_index=p_slot_index
    ...
    SELECT binding.* INTO source_binding
      FROM agent_runtime_media_action_bindings binding
     WHERE binding.output_message_id=message.id AND binding.action_index=p_slot_index
     ORDER BY binding.created_at DESC, binding.action_id DESC LIMIT 1 FOR UPDATE;
    IF source_binding.action_id IS NULL THEN
        RETURN jsonb_build_object('outcome','slot_not_found');
    END IF;
    SELECT * INTO source_action FROM agent_actions WHERE id=source_binding.action_id;
    SELECT * INTO source_task FROM tasks WHERE id=source_binding.task_id FOR UPDATE;
    IF source_action.status NOT IN ('failed','rejected','cancelled')
       OR source_task.id IS NULL OR source_task.delivery_context @>
          '{"actor":false,"runtime":true}'::JSONB IS NOT TRUE THEN
        RETURN jsonb_build_object(
            'outcome', CASE WHEN source_action.status='completed'
                            THEN 'slot_completed' ELSE 'slot_active' END);
    END IF;
    SELECT part INTO slot FROM jsonb_array_elements(message.content::JSONB) part
     WHERE part->>'type'='image'
       AND COALESCE(part->>'slot_index','-1')::INTEGER=p_slot_index LIMIT 1;
    IF slot IS NULL OR COALESCE(slot->>'slot_id','') !~
       '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
       OR COALESCE(slot->>'slot_revision','') !~ '^[0-9]+$' THEN
        RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_SLOT_CONTRACT_INVALID'
            USING ERRCODE = '55000';
    END IF;
    ...
    IF stable_slot_id IS DISTINCT FROM p_slot_id
       OR slot_revision IS DISTINCT FROM p_expected_slot_revision THEN
        RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_SLOT_REVISION_CONFLICT'
            USING ERRCODE = '40001';
    END IF;
    SELECT COALESCE(max(lineage.retry_ordinal),0)+1 INTO retry_ordinal
      FROM agent_runtime_media_retry_lineage lineage
     WHERE lineage.output_message_id=message.id AND lineage.slot_index=p_slot_index;
    action_id := gen_random_uuid();
    INSERT INTO agent_session_commands(
        session_id,org_id,user_id,command_type,idempotency_key,payload,request_hash
    ) VALUES(session.id,p_org_id,p_user_id,'submit_input',btrim(p_idempotency_key),
      jsonb_build_object('source','runtime_media_slot_retry','task_id',source_binding.chat_task_id,
    ...
    INSERT INTO agent_runs(
        session_id,command_id,org_id,user_id,run_kind,status,idempotency_key,
        request_hash,context_receipt,config_snapshot,capability_snapshot,
        blocking_action_count
    ) VALUES(session.id,command.id,p_org_id,p_user_id,'user','waiting_actions',
      ...
      jsonb_build_object('channel','web','runtime_media_slot_retry',TRUE),1)
      RETURNING * INTO run;
    UPDATE agent_session_commands SET result_entity_id=run.id WHERE id=command.id;
    INSERT INTO agent_model_steps(
        run_id,session_id,org_id,user_id,step_number,status,model_id,provider,
        model_revision,prompt_revision,tool_catalog_revision,request_receipt,
        response_receipt,stop_reason,completed_at
    ) VALUES(run.id,session.id,p_org_id,p_user_id,1,'completed',
    ...
    INSERT INTO agent_actions(
        ...
        wave,dependency_ids,blocking,policy_decision,policy_snapshot,policy_revision,
        retry_disposition,status
    ) SELECT action_id,session.id,run.id,step.id,p_org_id,p_user_id,0,
    ...
    INSERT INTO agent_policy_receipts(
        action_id,session_id,run_id,org_id,user_id,decision,arguments_hash,
        executor_type,executor_revision,policy_revision,effective_scope,
        reason_codes,receipt_hash,expires_at
    ) VALUES(action.id,session.id,run.id,p_org_id,p_user_id,'allow',
    ...
    IF final_balance IS NULL THEN
        RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_INSUFFICIENT_CREDITS' USING ERRCODE='P0001';
    END IF;
    request_params := (COALESCE(source_task.request_params, '{}'::JSONB) - '_task_slot_id')
      || CASE WHEN p_task_slot_id IS NULL
        THEN '{}'::JSONB ELSE jsonb_build_object('_task_slot_id',p_task_slot_id) END;
    ...
    INSERT INTO credits_history(user_id,change_type,change_amount,balance_after,description,org_id)
      VALUES(p_user_id,'image_generation_cost'::credits_change_type,
        -source_binding.unit_credits,final_balance,'Agent Runtime media slot retry reservation',p_org_id);
    INSERT INTO tasks(
        id,client_task_id,user_id,org_id,conversation_id,type,status,credits_locked,
        credits_used,model_id,placeholder_message_id,assistant_message_id,request_params,
        placeholder_created_at,input_message_id,turn_id,base_context_revision,
        context_through_message_id,execution_mode,delivery_context,image_index,batch_id,
        credit_transaction_id
    ) VALUES(action.id,COALESCE(NULLIF(btrim(p_client_task_id),''),
    ...
    INSERT INTO agent_runtime_media_action_bindings(
        action_id,slot_id,task_id,session_id,run_id,model_step_id,chat_task_id,org_id,user_id,
        conversation_id,batch_hash,action_index,action_arguments_hash,action_request_hash,
        input_message_id,output_message_id,credit_transaction_id,pricing_revision,
        pricing_model_id,pricing_resolution,pricing_fact_hash,provider_request_hash,
        unit_credits,reference_manifest_hash,projection_revision
    ) VALUES(action.id,stable_slot_id,action.id,session.id,run.id,step.id,source_binding.chat_task_id,
      ...
      slot_revision+1);
    INSERT INTO agent_runtime_media_retry_lineage(
        retry_action_id,source_action_id,root_action_id,output_message_id,org_id,user_id,
        conversation_id,slot_id,slot_index,retry_ordinal,base_slot_revision,idempotency_key
    ) VALUES(action.id,source_action.id,COALESCE((SELECT lineage.root_action_id
        FROM agent_runtime_media_retry_lineage lineage WHERE lineage.retry_action_id=source_action.id),
    ...
    UPDATE messages SET content=(SELECT jsonb_agg(CASE
        WHEN part->>'type'='image' AND COALESCE(part->>'slot_index','-1')::INTEGER=p_slot_index
        ...
        FROM jsonb_array_elements(message.content::JSONB) WITH ORDINALITY source(part,ordinality))::TEXT,
        status='pending',
        generation_params=jsonb_set(
            COALESCE(generation_params,'{}'::JSONB),
            '{runtime_media_batch,projection_revision}',
            to_jsonb(slot_revision+1), TRUE)
        WHERE id=message.id;
...
END $$;
CREATE FUNCTION read_agent_runtime_media_retry_binding_v1(
    p_action_id UUID, p_attempt_id UUID, p_worker_id TEXT,
    p_execution_token UUID, p_expected_attempt_version BIGINT, p_request_hash TEXT
...
BEGIN
    ...
    IF NOT _agent_runtime_media_attempt_valid_v1(p_action_id,p_attempt_id,p_worker_id,
       p_execution_token,p_expected_attempt_version,p_request_hash) THEN
        RAISE EXCEPTION 'AGENT_RUNTIME_MEDIA_ATTEMPT_SCOPE_INVALID' USING ERRCODE='42501';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM agent_runtime_media_retry_lineage
                   WHERE retry_action_id=p_action_id) THEN
        RETURN jsonb_build_object('outcome','not_retry');
    END IF;
