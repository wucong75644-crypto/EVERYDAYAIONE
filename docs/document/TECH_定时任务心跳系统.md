...
> 2026-09-12 候选更新：聊天任务管理改为主模型结构化参数，工具不再二次解析 description；面板 AI 创建一次提取后校验。原调度/执行/授权/通知继续复用。本文早期交互描述以 [ST-26 当前架构与验收](SCHEDULED_TASK_STRUCTURED_ACCEPTANCE.md) 为准；该候选未部署，新增真实模型复验待补。

> 2026-08-29 更新：本文 §4.4 的 Redis Pub/Sub 直推方案已退役，不再作为
...
新任务不再由任意入口直接写入 `scheduled_tasks.active`。创建链路统一为：
...
- Planner Agent 选择初始工具与验证步骤；系统只接受在当前租户可用工具集合内的 JSON 计划。
- 用户确认的是**工具/数据能力范围与结果契约**，不是将每一条未来调用写死。运行时模型仍能在此范围内重规划；范围外调用被执行器拒绝。
