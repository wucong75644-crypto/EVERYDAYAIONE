...
BEGIN
    IF EXISTS (SELECT 1 FROM agent_runtime_media_cancel_requests)
       OR EXISTS (SELECT 1 FROM agent_runtime_media_retry_lineage)
       OR EXISTS (SELECT 1 FROM agent_runs WHERE
            capability_snapshot @> '{"source":"runtime_media_retry",
              "execution_mode":"one_shot_action",
              "projection_mode":"media_action_only"}'::JSONB) THEN
...
DROP FUNCTION _agent_runtime_media_cancel_dispatch_v1(UUID,TEXT);
