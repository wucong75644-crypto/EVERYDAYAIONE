...
class TestBatchCompletionServiceHandleComplete:
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_single_task_complete_confirms_credits(self, mock_stream_pub, service, db):
        """测试：成功时确认积分"""
        batch_id = str(uuid4())
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_pushes_partial_update(self, mock_stream_pub, service, db):
        ...
        # 验证 WS 推送
        mock_stream_pub.assert_called_once()
        call_kwargs = mock_stream_pub.call_args
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_no_finalize_when_not_all_terminal(self, mock_stream_pub, service, db):
        ...
        assert mock_stream_pub.call_count == 1

    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_finalize_when_all_terminal(self, mock_stream_pub, service, db):
        ...
        assert mock_stream_pub.call_count == 2
        ws_calls = mock_stream_pub.call_args_list
        msg_types = [
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_no_transaction_id_skips_credit_confirm(self, mock_stream_pub, service, db):
...
class TestBatchCompletionServiceHandleFailure:
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_failure_pushes_error_partial_update(self, mock_stream_pub, service, db):
        ...
        # 验证推送带 error
        ws_msg = (
            mock_stream_pub.call_args.kwargs.get("message")
            or mock_stream_pub.call_args[1].get("message")
        )
        assert ws_msg["type"] == "image_partial_update"
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_failure_returns_true(self, mock_stream_pub, service, db):
        """测试：失败处理返回 True"""
        batch_id = str(uuid4())
        task = create_batch_task(index=0, batch_id=batch_id)
        db.set_table_data("tasks", [
...
class TestBatchCompletionServiceFinalize:
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_finalize_all_success(self, mock_stream_pub, service, db):
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_finalize_partial_failure(self, mock_stream_pub, service, db):
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_finalize_all_failed(self, mock_stream_pub, service, db):
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_finalize_empty_batch_noop(self, mock_stream_pub, service, db):
        """测试：空批次不做任何操作"""
        await service._finalize_batch("batch_empty", [])

        mock_stream_pub.assert_not_called()

    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_finalize_updates_conversation_preview(self, mock_stream_pub, service, db):
        """测试：finalize 更新对话预览"""
        batch_id = str(uuid4())
        batch_tasks = [
            create_batch_task(0, batch_id, status="completed",
...
class TestDispatchFinalize:
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_dispatch_to_finalize_batch_for_send(self, mock_stream_pub, service, db):
    ...
    async def test_dispatch_to_finalize_single_for_regenerate_single(
        self, mock_stream_pub, service, db
    ):
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_dispatch_defaults_to_batch(self, mock_stream_pub, service, db):
...
class TestFinalizeSingleImage:
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_success_merges_content(self, mock_stream_pub, service, db):
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_failure_marks_content_failed(self, mock_stream_pub, service, db):
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_all_failed_status(self, mock_stream_pub, service, db):
    ...
    @pytest.mark.asyncio
    @patch("services.batch_completion_service.stream_publish", new_callable=AsyncMock)
    async def test_empty_batch_noop(self, mock_stream_pub, service, db):
        """测试：空批次不做操作"""
