...
_DESCRIPTION_WORKSPACE = (
    ...
    "读写文件（必须用高性能引擎）：\n"
    "- 读 Excel: pd.read_excel('文件名.xlsx', engine='calamine')\n"
    "- 写 Excel: df.to_excel(OUTPUT_DIR + '/报表.xlsx', engine='xlsxwriter', index=False)\n"
    "- 读 Parquet: pd.read_parquet(STAGING_DIR + '/数据.parquet')\n"
    "- 写 CSV: df.to_csv(OUTPUT_DIR + '/数据.csv', index=False)\n"
    "- 图表: plt.savefig(OUTPUT_DIR + '/图.png', dpi=150, bbox_inches='tight'); plt.close()\n"
