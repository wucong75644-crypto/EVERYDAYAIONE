-- Restore pre-B4 cancellation only when no cancellation or in-flight operation depends on it.
SET LOCAL ROLE everydayai_owner;

DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM agent_runtime_model_gateway_operations o
  JOIN agent_runs r ON r.id=o.run_id
  WHERE o.terminal_error_code='GATEWAY_PARENT_RUN_CANCELLED_BEFORE_DISPATCH'
     OR o.ambiguity_code='GATEWAY_PARENT_RUN_CANCELLED_AFTER_DISPATCH'
     OR o.status IN('claimed','dispatching')
     OR (r.status='cancelled' AND o.status IN('submitted','claimed','dispatching'))) THEN
  RAISE EXCEPTION 'AGENT_MODEL_GATEWAY_CANCEL_FENCE_ROLLBACK_PENDING_FACTS';
 END IF;
END $$;
...
BEGIN
 ...
 IF o.lease_token IS DISTINCT FROM p_claim_token OR o.execution_token IS DISTINCT FROM p_execution_token
 ...
 IF o.status='dispatching' AND o.state_version=p_expected_operation_version+1 THEN
  RETURN jsonb_build_object('outcome','readback','operation',_agent_model_gateway_public(o)); END IF;
...
BEGIN
 ...
 SELECT * INTO o FROM agent_runtime_model_gateway_operations WHERE id=p_operation_id FOR UPDATE;
 IF NOT FOUND THEN RETURN jsonb_build_object('outcome','not_found'); END IF;
 IF p_lease_seconds NOT BETWEEN 15 AND 600 OR o.status NOT IN('claimed','dispatching')
 OR o.lease_token IS DISTINCT FROM p_claim_token OR o.execution_token IS DISTINCT FROM p_execution_token
 OR o.request_hash IS DISTINCT FROM p_request_hash OR o.provider_revision IS DISTINCT FROM btrim(p_provider_revision)
 OR o.tenant_kill_epoch<>p_tenant_kill_epoch OR o.provider_kill_epoch<>p_provider_kill_epoch
...
BEGIN
 ...
 END IF;
 IF o.status<>'dispatching' OR o.lease_token IS DISTINCT FROM p_claim_token
...
DROP FUNCTION _agent_model_gateway_parent_active_v1(UUID);
DROP FUNCTION _lock_agent_model_gateway_cancel_scope_v1(UUID);
