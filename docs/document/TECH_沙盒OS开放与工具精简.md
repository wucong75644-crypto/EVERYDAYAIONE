...
**改造前**：file_list 调 `extract_file_metadata()` 对前 5 个文件提取结构信息。
...
改造前：file_list 调 `extract_file_metadata()` 在工具返回中附加。
改造后：**不主动调用**。LLM 在 code_execute 内自然获取更准确的结构信息：
- `df.shape` → 精确行列数（不是采样估算）
- `df.dtypes` → 精确类型（不是推断）
- `df.head()` → 真实预览（不是格式化摘要）

最终实施状态（2026-07-18）：file_list/file_search 和 workspace prompt 均已停止调用旧提取器，file_read 也没有依赖该模块。全仓确认无生产调用后，`file_metadata_extractor.py` 及其孤立测试已删除。
...
> 实施修订（2026-07-18）：`file_metadata_extractor.py` 后续确认没有被 file_read 或其他生产链路使用，已作为退役死代码删除。
