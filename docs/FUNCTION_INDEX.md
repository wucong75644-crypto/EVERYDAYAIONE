...
| `read_agent_runtime_scheduled_finalization_context_v1` | `backend/migrations/227_33_agent_runtime_scheduled_finalization_context.sql` | Token-bound, side-effect-free Runtime Worker readback of the frozen schedule and retry counters needed to plan finalization; it revalidates the claimed lease, task/profile/binding/Run identity, epochs and revisions without exposing prompts, delivery targets, claim tokens or provider data. |
