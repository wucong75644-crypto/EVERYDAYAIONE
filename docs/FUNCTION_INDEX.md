...
| `read_agent_runtime_scheduled_wecom_dispatch_payload_v1` | `backend/migrations/227_46_agent_runtime_scheduled_wecom_dispatch_payload.sql` | Fenced worker-only gate over the approved 227_38 live dispatch context. A completed text item verifies exact run/task/finalization/content/item/version and frozen model-result identity, then requires the stored safe summary to equal a database-side re-sanitization of that exact result. It returns only the safe text, opaque hashes, and verified tenant transport identity: App `org_id`/`corp_id`/`wecom_userid` or Smart Robot `org_id`/`chatid`; `org_id` is included in the canonical payload hash. Artifact and non-completed items return fixed payload-free `unsupported`; raw model output, snapshots and internal IDs never leave the RPC. It inherits 227_38's pre-attempt unavailable cancellation without passing through the full context response. |
...
| `recover_agent_runtime_scheduled_wecom_started_dispatch_v1` / started-recovery request ledger | `backend/migrations/227_48_agent_runtime_scheduled_wecom_started_recovery.sql` | Selects only an expired, exact `dispatch_started/external_request_started` attempt whose delivery/item/claim/provider/frozen versions and live org/target context still match. In one transaction it generates a distinct internal outcome request and invokes the authoritative 227_40 UNKNOWN transition with no evidence, then stores an append-only FORCE-RLS recovery receipt for durable response-loss readback. The attempt becomes unknown/ambiguous and reconcile-only; no prepare, start, transport, new attempt or resubmit occurs, and the recovery request joins the full global request namespace bidirectionally. |
