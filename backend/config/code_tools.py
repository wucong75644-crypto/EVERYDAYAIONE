...
_DESCRIPTION_WORKSPACE = (
    ...
    "目录：WORKSPACE_DIR（用户文件）, STAGING_DIR（ERP 数据缓存）, OUTPUT_DIR（输出，自动上传）, "
    "SKILLS_DIR（文件处理指南，只读）。\n"
    "处理文件前，先读取对应的处理指南：\n"
    "  with open(SKILLS_DIR + '/excel.md') as f: print(f.read())  # Excel\n"
    "  with open(SKILLS_DIR + '/csv.md') as f: print(f.read())    # CSV/TSV\n"
    "  with open(SKILLS_DIR + '/pdf.md') as f: print(f.read())    # PDF\n"
    "  with open(SKILLS_DIR + '/docx.md') as f: print(f.read())   # Word\n"
    "  with open(SKILLS_DIR + '/pptx.md') as f: print(f.read())   # PPT\n"
    "读取指南和探索文件结构可以合并在同一次 code_execute 中完成。\n"
    "ERP 查询结果在 STAGING_DIR（Parquet），用 duckdb.sql() 查询。\n"
