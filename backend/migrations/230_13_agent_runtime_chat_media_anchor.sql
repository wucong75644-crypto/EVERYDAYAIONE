-- 230.13: close the Chat Action -> Runtime media preparation seam.
-- Chat tool Actions reuse the existing Runtime media batch/Provider path.  The
-- command must therefore carry the chat task's input and output anchors.
...
BEGIN
    ...
    -- A media Action is not executable until the original chat task and its
    -- user input anchor are proven in the same tenant/conversation scope.
    IF p_tool_name = 'generate_image'
       AND COALESCE(pg_input_is_valid(p_task_id, 'uuid'), FALSE) THEN
        SELECT task.input_message_id INTO input_message_id
          FROM tasks task
         WHERE task.id = p_task_id::UUID
           AND task.type::TEXT = 'chat'
           AND task.user_id = s.user_id
           AND task.org_id IS NOT DISTINCT FROM s.org_id
           AND task.conversation_id = s.conversation_id
           AND task.assistant_message_id::TEXT = p_message_id;
        IF input_message_id IS NULL THEN
    ...
    INSERT INTO agent_session_commands(
        ...
        jsonb_build_object(
            'source', 'chat_action', 'task_id', p_task_id,
            'input_message_id', input_message_id,
            'output_message_id', p_message_id,
    ...
    INSERT INTO agent_actions(
        ...
        p_tool_name, p_arguments, h, encode(sha256(convert_to(
            ...
            'UTF8')), 'hex'), h, 'preauthorized',
        p_policy_snapshot || jsonb_build_object(
            'task_id', p_task_id,
            'input_message_id', input_message_id,
            'output_message_id', p_message_id
        ),
