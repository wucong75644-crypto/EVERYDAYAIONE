"""Generate the immutable Runtime v13 image-only release seed."""
...
TARGET = ROOT / "migrations/230_06_agent_runtime_catalog_image_v13.sql"


def main(target: Path = TARGET) -> None:
    snapshots = [
        build_image_snapshot(scope=scope, channel=channel, gate_state=gate)
    ...
    default = next(
        ...
        if item.toolset_document["scope_kind"] == "user"
    ...
    target.write_text(f"""-- S3 generated Runtime image-only release. Do not edit facts by hand.
-- Source: services/agent/runtime/catalog/image_release.py
...
INSERT INTO agent_runtime_definition_facts(
  ...
  {sql_text(catalog_revision)},{sql_text(default.toolset_hash)},
