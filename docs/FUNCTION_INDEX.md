...
| `RuntimeTaskCancelService` / `PostgresRuntimeTaskCancelRepository` | `backend/services/agent/runtime/task_cancel.py`、`infrastructure/postgres/task_cancel_repository.py` | Web Runtime task cancel typed边界：严格分类Owner和持久绑定，只调用v2 facade；冲突与不确定receipt失败关闭且不暴露底层异常 |
