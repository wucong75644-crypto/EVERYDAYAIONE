...
class SkillRuntime:
    ...
    async def initialize(self, checkpoint: dict | None = None):
        ...
        for candidate in candidates:
            if not candidate.catalog_metadata.model_selectable:
                continue
    ...
    def messages(self) -> list[dict[str, str]]:
        messages = []
        if self.directory:
            messages.append({"role": "system", "content": self._directory_text(self.directory.values())})
    ...
    async def activate(self, raw_arguments: str) -> dict:
        self._check_cancelled()
        try:
    ...
    async def _restore(self, raw):
        try:
            ...
            if len(directory) != len(checkpoint.directory) or any(
                not c.catalog_metadata.model_selectable for c in directory.values()
            ):
...
async def create_skill_runtime(*, handler, context, runtime, replay_context=None):
    ...
    await state.initialize(checkpoint)
