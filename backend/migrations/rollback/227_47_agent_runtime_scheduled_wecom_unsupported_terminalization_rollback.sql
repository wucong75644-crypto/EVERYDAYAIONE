-- Roll back 227_47 only when no durable request or attributable terminalization exists.

SET LOCAL ROLE everydayai_owner;

LOCK TABLE agent_runtime_scheduled_wecom_unsupported_requests IN SHARE ROW EXCLUSIVE MODE;
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM agent_runtime_scheduled_wecom_unsupported_requests)
 OR EXISTS(SELECT 1 FROM agent_runtime_scheduled_wecom_delivery_items
  WHERE status='cancelled' AND terminal_reason_code IN(
  ...
  RAISE EXCEPTION 'AGENT_RUNTIME_SCHEDULED_WECOM_UNSUPPORTED_ROLLBACK_HAS_FACTS' USING ERRCODE='55000';
 END IF;
END $$;

REVOKE ALL ON FUNCTION terminalize_agent_runtime_scheduled_wecom_unsupported_item_v1(
 UUID,UUID,UUID,UUID,UUID,TEXT,BIGINT,BIGINT) FROM everydayai_wecom_runtime;
DROP FUNCTION terminalize_agent_runtime_scheduled_wecom_unsupported_item_v1(
 UUID,UUID,UUID,UUID,UUID,TEXT,BIGINT,BIGINT);
DROP FUNCTION _agent_runtime_scheduled_wecom_unsupported_json(
...
BEGIN
 ...
 OR EXISTS(SELECT 1 FROM agent_runtime_scheduled_wecom_reconcile_claim_requests WHERE request_id=NEW.request_id)
 OR EXISTS(SELECT 1 FROM agent_runtime_scheduled_wecom_continuation_claim_requests WHERE request_id=NEW.request_id)
 OR EXISTS(SELECT 1 FROM agent_runtime_scheduled_wecom_reconcile_result_requests WHERE request_id=NEW.request_id) THEN
...
DROP TRIGGER runtime_scheduled_wecom_unsupported_global_request_guard
 ON agent_runtime_scheduled_wecom_unsupported_requests;
DROP FUNCTION _agent_runtime_scheduled_wecom_unsupported_request_guard();
DROP TRIGGER runtime_scheduled_wecom_unsupported_immutable
 ON agent_runtime_scheduled_wecom_unsupported_requests;
DROP FUNCTION _agent_runtime_scheduled_wecom_unsupported_immutable();
DROP TABLE agent_runtime_scheduled_wecom_unsupported_requests;
