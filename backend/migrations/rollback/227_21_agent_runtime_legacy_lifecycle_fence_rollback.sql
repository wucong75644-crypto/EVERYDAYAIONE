-- Roll back AR-18-A1.1 only when no non-terminal Runtime-owned task can be
-- exposed to the restored legacy lifecycle owners.
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'AGENT_RUNTIME_LEGACY_LIFECYCLE_ROLLBACK_ACTIVE_RUNTIME_TASKS'
            USING ERRCODE = '55000';
    END IF;
END;
$$;
...
BEGIN
    ...
    ELSE
        INSERT INTO public.messages(
            ...
            v_task.org_id,
            'assistant', p_content::TEXT, 'interrupted', 0, FALSE,
            jsonb_build_object(
                'type', COALESCE(v_task.type, 'chat'),
                'model', COALESCE(v_task.model_id, 'unknown'))
        );
...
BEGIN
    ...
    UPDATE public.tasks
     ...
     WHERE id = v_task.id;
    RETURN jsonb_build_object(
        'outcome', 'failed', 'task_id', v_task.id, 'refund', v_refund);
END;
$$;
    ...
    IF p_message_content IS NOT NULL
       AND v_task.placeholder_message_id IS NOT NULL THEN
        INSERT INTO public.messages (
            ...
            v_task.placeholder_message_id, v_task.conversation_id,
            'assistant', p_message_content, 'failed', 0,
