-- 228.08l: allow the narrow Web Runtime failure finalizer to repair the
-- historical half-terminal state produced when startup recovery failed the
-- task but left its assistant placeholder active.
...
BEGIN
    ...
    IF task.status = 'failed' AND output_message.status::TEXT = 'failed'
       AND task.delivery_context->>'runtime_failure_code' = p_failure_code THEN
    ...
    IF output_message.status::TEXT NOT IN ('pending','streaming','generating')
       OR NOT owner_unresolved
       OR NOT (
           task.status IN ('pending','preparing')
           OR (
               task.status = 'failed'
               AND task.terminal_reason = 'startup_recovery_failed'
           )
       ) THEN
    ...
    END IF;
    UPDATE tasks SET
        status = CASE WHEN task.status IN ('pending','preparing')
            THEN 'failed' ELSE task.status END,
        completed_at = CASE WHEN task.status IN ('pending','preparing')
            THEN clock_timestamp() ELSE task.completed_at END,
        terminal_reason = CASE WHEN task.status IN ('pending','preparing')
            THEN 'runtime_ingress_failed' ELSE task.terminal_reason END,
        error_message = CASE WHEN task.status IN ('pending','preparing')
            THEN '生成服务暂未就绪，请稍后重试' ELSE task.error_message END,
