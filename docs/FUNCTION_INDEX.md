...
| `create_agent_runtime_scheduled_execution_profile_v1` / `read_agent_runtime_scheduled_execution_profile_v1` | `backend/migrations/227_29_agent_runtime_scheduled_execution_owner.sql` | Freeze the explicit new Runtime task's tenant, source Action/Run, definition/model, enabled unattended-safe toolset, scope, channel, budget and revision facts; historical tasks remain legacy. |
| `select_agent_runtime_scheduled_run_owner_v1` / `read_agent_runtime_scheduled_run_owner_v1` | `backend/migrations/227_29_agent_runtime_scheduled_execution_owner.sql` | Atomically choose one monotonic legacy/runtime owner per scheduled trigger, with task/run/tenant/revision/hash/kill binding and response-loss readback. |
| `bind_agent_runtime_scheduled_run_runtime_v1` / `assert_agent_runtime_scheduled_run_owner_v1` | `backend/migrations/227_29_agent_runtime_scheduled_execution_owner.sql` | Reserve unique Runtime Command/Run binding and provide the future owner gate without creating Commands/Runs or granting the legacy worker new authority. |
