...
_DESCRIPTION = (
    ...
    "\n"
    "DATA HELPERS (沙盒已注入 globals,直接调用)\n"
    "  safe_float(v, default=0.0) -> float  # 处理混合类型:'47.40%'→0.474,'-'/NaN→default,千分位/货币符号剥离\n"
    "  safe_int(v, default=0) -> int        # safe_float + 取整\n"
    "  用法: df['x_num'] = df['x'].apply(safe_float)  # ragged 混合列一行处理\n"
    "  错误: 直接 float('47.40%') → ValueError\n"
    "\n"
    "CAVEATS\n"
