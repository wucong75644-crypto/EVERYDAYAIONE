...
_DESCRIPTION_BASE = (
    ...
    "环境变量: STAGING_DIR（staging数据目录）, OUTPUT_DIR（自动上传）\n"
    "数据文件已由 file_analyze 转为 Parquet，直接用 STAGING_DIR + '/parquet文件名' 拼接路径。\n"
    "示例：df = duckdb.sql(f\"SELECT * FROM read_parquet('{STAGING_DIR}/_cache_a1b2c3_sheet0_销售报表.parquet')\").df()\n"
...
_DESCRIPTION_WORKSPACE = (
    ...
    "file_analyze 返回的提示词里有 STAGING_DIR + '/parquet文件名'，直接复制使用。\n"
    "示例：df = duckdb.sql(f\"SELECT * FROM read_parquet('{STAGING_DIR}/_cache_a1b2c3_sheet0_销售报表.parquet')\").df()\n"
    "OUTPUT_DIR 存输出文件，自动上传。\n"
    "数据文件（Excel/CSV）先调 file_analyze，再用 STAGING_DIR + parquet文件名 + duckdb 查询。\n"
