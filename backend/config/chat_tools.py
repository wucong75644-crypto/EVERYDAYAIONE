...
【产物输出协议 — 想给用户看的内容必须调 emit_xxx,不能只 print 文字】
- emit_chart(option, title='')   ECharts 图表(option 完整配置 dict)
- emit_file(path, label=None)    文件下载卡片(写文件后必须 emit)
...
禁止: 旧的 open('staging/x.echart.json', 'w') 协议已废弃,前端不会再渲染这种文件,
      必须用 emit_chart(option) 让 LLM 输出的图表配置真正显示给用户。

数据文件已 file_analyze 治理过的,attachments 会有 parquet 字段,字面 copy 即可。
列名用双引号包裹。
图表配置用 ECharts JSON dict,通过 emit_chart 输出,不要用 matplotlib。
