...
# ============ Versioned ConversationCache 自动 mock ============
...
def _auto_mock_conversation_cache(monkeypatch):
    """测试默认让闭合历史缓存 miss，避免依赖 Redis 实际数据。"""
    try:
        ...
        monkeypatch.setattr(_cc, "get_closed_messages", _noop_get)
        monkeypatch.setattr(_cc, "set_closed_messages", _noop_set)
