    ...
    return [
        {
            ...
            "function": {
                ...
                "description": (
                    ...
                    "pandas(pd), io\n\n"
                    "典型流程：\n"
                    "1. local_db_export / fetch_all_pages 获取数据 → staging/xxx.jsonl\n"
...
CODE_ROUTING_PROMPT = (
    ...
    "大数据会自动存到 staging 文件\n"
    "- 典型流程：local_db_export 拿数据 → code_execute 用 read_file 读取 → "
    "pd.read_json(io.StringIO(raw), lines=True) → pandas 计算 → upload_file 导出\n"
    "- staging 文件是 JSONL 格式（每行一条 JSON），"
    "用 pd.read_json(io.StringIO(raw), lines=True) 读取\n"
