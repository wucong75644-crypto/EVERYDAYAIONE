...
- 2026-09-23 / Skill 通用运行机制（本地实现，未部署）：原测试附件已完整加载，但旧空工具名单阻断取数，回答还受历史话题及模式规则冲突影响。按已确认方案实现新建 Skill 使用当前会话权限、旧版显式升级与审核、完整方法绑定当前请求、实际工具能力提示、模式规则统一及旧快照兼容；没有店铺专用分支。后端定向回归 1,145 项、前端 58 项及类型/ESLint 通过，最终 Qwen 合成模型场景 6/6 通过；不代表任意自然语言流程的确定性保证。原生产 Skill 的内容和权限未更改，业务验收须部署后完成。[实现、文件、模型证据、生产验证与回滚](document/TECH_Skill统一运行机制.md)。

- 2026-09-22 / 数据对比展示契约（本地完成，未再次部署）：首版 `66c9b933` 移除 span 标签后丢失颜色，用户验收未通过；随后本地箭头推色方案被否定、未发布且已移除。现用单一 JSON 颜色目录约束模型说明、工具能力、后端校验和前端渲染：正文支持受控行内颜色、兼容历史合法 span；结构化表格通过可选 cell_styles 传递颜色/加粗，原始数值与样式分离，完整覆盖 emit、持久化、WS/历史解析及展示，不再从数值或箭头推断颜色。新增回归在修改前复现前端 2 项、后端 1 项失败；最终前端全量 1456 passed（4 workers），后端相关 660 passed、2 项本机缺少 Linux 字体 skip，定向 ESLint、类型检查、生产构建、真实 kernel IPC 和明暗模式浏览器核验通过。默认高并发的两次前端运行各有 1 项不同异步等待失败（Skill 弹窗 / ECharts 重载），未修改这些断言，限制并发后全量通过。**本轮尚未提交部署，真实模型生产会话及用户验收待完成**。见 [契约与验证](document/TECH_结构化消息运行时边界与渲染安全.md#8-2026-09-22-展示契约修复)。

- 2026-09-20 / 图片任务15分钟超时（本地，未部署）：针对本地10分钟超时而KIE随后成功的任务，按确认将后端图片清理/兜底超时复核与前端刷新恢复常量统一为15分钟；不改CDN主链路、海外旁路、400单次重提、积分结算及其他类型任务超时，不自动恢复历史失败。后端185项、前端15项测试通过，覆盖10至15分钟等待、13分24秒回调/轮询成功、15分钟边界及最终结算。生成服务变慢、创建ConnectTimeout、缓存等待和服务重启中断不由本次修改解决。见 [KIE状态与一致性](document/TECH_KIE图片旁路重试.md#状态与一致性)。

- 2026-09-21 / Skill 删除被历史暂停任务误拦：生产 `705d3044` 上“测试日报”显示 36 个未知依赖。只读确认实际引用 0；36 个暂停快照最晚 09-18，Skill 创建于 09-20，旧检查把缺少 Skill 记录统一算未知。新增迁移 262，仅排除仍暂停、同 Turn、快照严格早于包创建、没有选择或 Skill 记录的历史任务；实际引用、已恢复任务和异常记录继续拦截。50 项临时 PostgreSQL 定向测试通过；以实际生产数据库角色只读执行候选查询，未知数从 36 降至 0，业务数据未改。**修复尚未部署**。[文件、验证、上线复验与回滚](document/TECH_Skill草稿审核与发布.md#历史暂停任务误拦删除修复2026-09-21未部署)。

- 2026-09-21 / Skill 停用、废弃与删除规则：用户确认 B（停用可直接废弃），废弃后只读并支持安全删除。已实现迁移 261、历史保留的删除标记、组织权限和依赖检查、事务内复查、删除后写入保护及界面；未知引用/存储/数据库失败关闭操作。独立审查发现的并发写入窗口已用统一包锁和 draft 行锁修复，复审通过。后端 152 项、前端 34 项及类型/定向 ESLint 通过。**本次未部署**。[实现、文件、测试、生产验证与回滚](document/TECH_Skill草稿审核与发布.md#停用直接废弃与安全删除2026-09-21未部署)。

- 2026-09-20 / Skill 停用后无法重新启用：前端、动作契约和数据库均把 disabled 当终态，缺少恢复路径。已补详情主按钮与基于本次停用审计的原子恢复（260，含保留数据回滚），原正文与 revision 不变，不恢复已废弃版本为 published 或补回撤销授权。后端 137 项、前端 24 项及类型/定向 ESLint 通过，**未部署，生产复验待完成**。[根因、验证与回滚](document/TECH_Skill草稿审核与发布.md#停用后重新启用修复2026-09-20未部署)。

- 2026-09-19 / Skill P1-4 手动调用：生产日志确认 Skill 已激活且生成 4,897 字符，Gateway 将整段输出在 60 秒截断；消息路由、失败交付与持久化又丢失激活反馈。已在原任务工作树修复预算/空闲超时、完整消息显示及原子失败快照（258，含回滚），后端 266 项、前端 59 项及构建通过；5 项既有外部数据库测试跳过。**未部署，生产复验待完成**。[证据、文件与验证](document/TECH_Skill聊天选择与反馈.md)。

- 2026-09-19 / KIE图片链路恢复（本地，未部署）：按用户确认，从最新稳定基座局部恢复CDN首次提交，以及7890下载CDN、7891后台上传、400单次临时链接兜底；不恢复国内上传、不读取工作区源文件、不回退其他新功能。核心106项及相关170项模拟测试全部通过，合计276项。生产发现的工作区读取ValueError与提交ConnectTimeout是不同问题，本次绕开工作区读取，不宣称修复代理连接异常或保证成功率。详见 [KIE链路设计与验证](document/TECH_KIE图片旁路重试.md)。

- 2026-09-14 / F08-05 默认范围：用户明确普通聊天默认查工作区和当前对话历史附件。已将 interactive file_search 缺省统一为获准工作区，复用上传存储，显式 current/定时预检/群隔离/删除确认保留；新增 23 项、相关回归通过。未提交部署；名字改写独立问题和整体验收仍未关闭。[实现记录](document/TOOL_UNIFICATION_DEFAULT_SEARCH_SCOPE_08.md)。

- 2026-09-13 / F08-05 最新真实模型复验：已追加完成 24 次；说明调整候选仍有 5/12 未通过（原会话漏搜 3、完整路径改写 1、范围未澄清 1）。原文件未变、业务写入 0，候选停止发布；下一步需单独明确原文到资源身份的结构化绑定合同。见 [真实模型证据](document/TOOL_UNIFICATION_FILE_PROTOCOL_MODEL_08.md)。下条“未验证/待授权”为此前快照。

- 2026-09-13 / F08-05：生产模型把完整文件名改写后走 keyword，并有一次遗漏 scope；原文件存在，删除 Handler 未调用。原任务已完成完整名字接现有定位器、显式模型 scope、歧义不降级的代码候选，本地相关 1735 节点有效通过（新增 29）。真实模型复验未验证，未提交部署，问题保持未关闭；F08-03/F08-04 定时任务模型问题不在此次修复范围。见 [修复记录](document/TOOL_UNIFICATION_FILE_PROTOCOL_FIX_08.md)。

- 2026-09-12 / ST-27：80cb123e 完整聊天请求越过用户确认直接创建。已改为始终先出可编辑表单，点击确认才进入原提交链；取消不创建。后端 1319、前端 37 通过，**本批未部署**。见 [ST-27](document/SCHEDULED_TASK_CONFIRMATION_ACCEPTANCE.md)。

- 2026-09-12 / ST-26：按用户确认的 Grok Build 分工，在原任务链路接入主模型结构化输入、补丁更新及共享字段校验；补齐渠道选择、未命中 ID、日历 cron 一致性。后端 1685 + 临时 PG 27 项通过，前端相关测试和构建通过；**新增真实模型协议联调未验证，未提交部署/用户验收**。详见 [ST-26 架构与验收](document/SCHEDULED_TASK_STRUCTURED_ACCEPTANCE.md)。

- 2026-09-12 / `d16adf19` 完整口语创建请求显示空白执行内容：获准真实 Qwen 复现证明模型输出正确，但来源校验拒绝“八点钟/发给我看”后删除 prompt。已在原校验层修复，修改后两次真实解析无缺项；1072 项定向回归通过，三次原始输出均验证到既有创建提交边界。**本批未提交部署，生产用户验收待完成**。见 [ST-25 证据及限制](document/SCHEDULED_TASK_COLLOQUIAL_ACCEPTANCE.md)。下方 ST-19～24 已随 d16adf19 完整部署，旧“未部署”为历史快照。

- 2026-09-12 / `d851d93e` 创建表单未选执行频率仍发送请求：补齐 FormBlock 可见必填项校验，保留输入，后端缺频率改中文提示；前端 30、后端 86、TS/定向 ESLint 通过。**本批未部署**。见 [ST-24 增补](document/SCHEDULED_TASK_FEEDBACK_ACCEPTANCE.md#8-st-24未选频率的创建请求2026-09-12)。

- 2026-09-12 / `002286f0` 用户复验 ST-19～23：通知选项隐藏、运行产物混合类型未展示、创建静态检查提示、聊天最终保存丢弃删除/暂停 ChangeSet 卡片、面板操作无成功文字。已增量修复，后端 478、前端 146、类型/构建通过；1 项补充 lint 在基准同样失败。**本批尚未提交部署，生产删除请求仍待用户确认**。消息类型回退须保留兼容补丁，详见 [验收与回退证据](document/SCHEDULED_TASK_FEEDBACK_ACCEPTANCE.md)。下方先前修复已随 `002286f0` 部署，“未部署”为当时快照。

- 2026-09-12 / 定时任务二次复查 ST-14～18：修复创建草稿串值、NL 收件对象未绑定、连续澄清丢失业务要求、时钟值与原文不一致仍通过、ChangeSet 漏事件后不刷新。后端 1032（含临时 PG 27）、前端 141、TS/构建及 6 条保存模型输出重放通过。**尚未部署、生产任务未改，用户验收未关闭**。见 [本批验收](document/SCHEDULED_TASK_CONSISTENCY_ACCEPTANCE.md)。

- 2026-09-12 / `eeccf3bd` 生产复验：任务执行内容被覆盖为整段创建原话，且补全表单把它隐藏。已在严格解析内分离管理信息与业务要求，保留原文中的日期、范围、口径、分组、筛选和输出条件；补全时可见可改，无法整理时不再预填整个请求。后端 696、前端 194、类型检查及 6 条真实模型合成请求通过，**本轮尚未部署，截图旧任务未改，用户验收仍未关闭**。08:00 经核实来自表单提交，非自动补值。见 [ST-11～13 验收](document/SCHEDULED_TASK_CONTENT_EXTRACTION_ACCEPTANCE.md)。
...
- 2026-09-11 / 05 附件读取：1df0e01c 生产中附件已落盘且进入模型上下文，模型仍误报无附件或搜索上一份文件。已补当前 user 消息中的文件身份绑定；774 项回归、6 次真实模型合成调用通过。首次发布后端测试失败，修正契约断言后待重新完整发布、原生产复验未关闭，不能宣布根治。证据与验收见 [附件绑定记录](document/TOOL_UNIFICATION_05_ATTACHMENT_BINDING.md)。

> 本文档记录项目中当前存在的已知问题、待修复的Bug、技术债务等。

## 问题状态

### 2026-09-11 板块 05 生产验收：上下文机制已恢复，真实模型及用户验收未关闭

- 用户随后已授权恢复上下文架构相关逻辑。已补齐独立交付事实、缓存投影隔离、逐调用结果状态及唯一动态摘要，并在总预算、历史预算和循环摘要中保留完成事实。原代码 44 项新回归失败，当前全部通过；相关共 986 passed、3 项既有 skip、0 failed。真实模型与用户生产复验仍未完成，整块技术验收未通过。见 [最新恢复与验收证据](document/TECH_05_上下文结果恢复.md)。

- `887b28ae` 上两次删除都被 `scheduled_task_drafts_confirmed_task_id_fkey` 拒绝；新建 ChangeSet 的任务成功，失败对象是另一个带旧草稿引用的任务。增加 253 迁移，沿用 `source_task_id` 已有的级联生命周期；保留独立 ChangeSet 审计/幂等回执。
- 读取新工作区附件时，模型把文件名填进了仅接受签名引用的 `resource_ref`。附件、搜索、工具描述现共享可复制调用契约，分别仅给 file_id 或正式 resource_ref；删除旧位置参数示例，无效引用仍严格拒绝。真实模型首次选参待复验。
- Actor 固定的新输入确实是“读取文件”，未错取旧输入；实际模型却继续历史导出。Git 探针确认 7 月正文压缩与 8 月表单终止叠加使无正文结果消失；本轮已修结果保留与失败状态失真，Actor 输入和表单终止保留。其对真实模型目标误用的因果贡献尚未证实，不宣称已根治该现象。此前位置/目标提示补丁仍未恢复。
- 生产只做了日志和只读事务查询，未执行删除、迁移或额外模型/付费媒体调用；保留原任务，未进入 06。详见 [修复及验证证据](document/TOOL_UNIFICATION_05_PRODUCTION_FIXES.md)。

### 2026-09-10 工作区搜索后分析失败 — 本地根因修复通过，待部署/用户验收

- 当前候选 bb4449a0 已完整部署，前端 1309、后端 9149 测试通过（另 37 skipped、4 xfailed），健康检查通过。下方“提交部署中/旧候选”均为当时快照。
- 用户生产验证发现 search 使用 workspace 后，后续目录搜索和 analyze 漏传 scope，回落 current 附件范围；三种文件选择器均被 RESOURCE_PATH_NOT_IN_MANIFEST 拒绝。未进入分析 Handler，但模型反复换文件并给出错误恢复建议；100 项截断列表被误述为总数。
- 已在原任务补齐可信资源动作上界、跨调用定位线索、同批调用快照、Actor 同任务恢复与错误止损。三入口统一检查，明确 current 不扩大；图片搜索也需 read；定时仅有工具名而无明确资源清单时拒绝，现有模板字段提供精确只读范围。
- 同一套生产断点复现：bb4449a0 隔离副本 4 failed，修复后 4 passed。最终相关回归 2960 passed、2 项原有真实模型 opt-in 未验证，源码兼容检查通过。本轮未提交部署或改生产数据，用户验收仍未关闭，不进入 05。见 [最新逐项验收](document/TOOL_UNIFICATION_ACCEPTANCE_04.md)、[实现与限制](document/TECH_工作区资源选择与授权衔接.md)。
- 长引用、分页及把截断数量误述为总数属于 05/06 结果消费问题，本轮没有改造展示/replay，不声称已解决。

### 2026-09-10 文件目标解析、确认版本与内容缓存 — 技术验收通过，提交部署中

- 发布增量：首轮 ba357093 后端全量门禁 4 项过时测试失败，前端已部署、后端未同步，完整候选无效。已修正精确路径 fixture/缓存版本断言并 114 项定向复验通过，等待本次重试的完整发布结果。

- 生产按名称/片段删除失败的原因是名称被当精确路径、根路径猜测及确认/执行分别解析目标；不是正确资源 URL 在传输中丢失。修改前隔离复现另证实明确目录被缓存同名替换、冷 fid 无法定位、ghost 清单与文本反解析引用错误、确认期间内容替换仍进入 Handler、短任务 ID 取首条、恢复覆盖和 CSV 尾部缓存过期缺陷。
- 在工具统一 04 原任务分支修复：统一当前 owner 范围解析与签名资源引用，文件存在性/唯一性/版本先于确认和 invocation，最终工作区锁内复核；所有必要源文件 writer、上传/OSS 和内核取消收尾纳入协调。恢复固定 record_id/ETag 并拒绝覆盖；全内容/解析配置缓存与调用视图来源分离；任务短 ID 歧义不提交提案。
- 本轮最终本地 2904 passed，2 项原有真实 LLM/私有文件 opt-in 测试未执行。当前 NAS 同机 flock 及不覆盖发布临时文件探针均已验证；用户提交部署指令后补齐存储证据，技术验收通过。NFS local_lock=all 仅支持当前同机多 worker；历史 deleted_files 无不可变备份版本，不能由当前 ETag 倒推历史数据。
- 生产当前仍为旧候选 `20636929f7b78346991b357630240d932ed5772d`；本轮未提交部署，无生产业务删除。详见 [04 最新验收记录](document/TOOL_UNIFICATION_ACCEPTANCE_04.md)、[实施方案](document/TECH_文件目标解析与确认闭环.md)。用户验收关闭前不进入 05。


### 2026-09-09 文件工具资源边界缺口 — 本地已修复，待提交部署

- 旧 `file_delete` 对绝对路径及缓存/ID 命中结果没有统一工作区校验；旧 `code_execute` 输出登记在边界校验之前就登记 basename，允许工作区外路径进入缓存。两处源码与已部署候选相同。
- 隔离诊断使用真实路径守卫、原 Handler 和临时文件，删除/记录全部 mock；外部路径模拟删除调用 1 次、边界检查 0 次，外部 stdout 路径被登记。没有执行真实删除。正常危险操作确认仍存在，缺口在确认后的资源路径校验。
- 已按用户授权核清调用链后修复：删除统一预检真实目标，保留 OSS 副本和原恢复记录；stdout 与当前会话 staging 产物登记前校验。41 个新边界/兼容测试通过，最终主组与 ERP 合计 1492 passed、4 原有 xfailed。详见 [修复验收](document/FILE_TOOL_BOUNDARY_REPAIR.md)，[原发现快照](document/OTHER_TOOL_LOGIC_AUDIT.md)。尚未提交部署。

### 2026-09-09 工作区附件 ID 查找失败 — 本地修复，待提交部署

- 先搜索再插入附件时，文件缓存按归一化文件名去重，漏登记完整路径，导致正确 file_id 反查失败；不同目录同名文件还可能串用路径与分析状态。
- 缓存改为按工作区实际路径合并，完整路径与名字别名分离，歧义不任意命中；分析状态按完整源路径写回/读取，淘汰同步清除所有索引。没有修改文件 ID 哈希、WS、持久化或切换工具统一生产入口。
- 同一套 21 个新增测试在已部署基准全部失败，修复后全部通过；相关回归 1111 passed，4 项原有 xfailed 已在基准复现。详见 [方案与验收记录](document/FILE_CACHE_ID_REPAIR.md)。当前尚未提交部署，需新候选生产验证。

各项修复及生产验证状态以对应记录为准。

### 2026-09-08 KIE 工作区旁路读取边界 — 本地修复，未部署

- 未发布的取图优化原先只限制工作区总目录，缺少用户归属校验；同时拒绝全部 query/fragment，误拦不改变原图内容的链接。此前顺序和路径测试未覆盖这两个边界，审查阶段通过模拟和调用链核对发现，未发生本次改动的生产发布。
- 修复限定在旁路读取及必要的身份透传：复用后端用户/组织或已有可信频道作用域与现有文件归属规则，缺身份和越权不读取；仅放行明确的缓存/鉴权参数，忽略 fragment，拒绝图片处理和文件版本参数。
- 本地相关回归 248 项通过，独立安全复查通过；图片顺序、上传出口、生成参数和兜底重试流程不变，不恢复 CDN 下载。详见 `docs/document/TECH_KIE图片旁路重试.md`；尚未提交部署。

### 2026-08-29 快麦 ERP 告警与归档噪声修复 — 开发完成，待部署

- ERP 健康告警指纹不再包含连续失败次数；同一同步类型在 1 小时去重窗口内只推送一次，避免持久故障每增加 3 次失败就重复告警。
- `error_logs` 指纹忽略 `consecutive_errors` 和“X 次失败”的运行时计数；调度器正常轮询改为 INFO，`platform_map` 的可重试网络/未知异常改为 WARNING。
- 归档前统一将 `exception_tags` 编码为 PostgreSQL `TEXT[]` 字面量，覆盖 WorkerPool 和 Redis 不可用时的旧 Worker 入口，修复 `malformed array literal`。
- 当前只完成代码和回归测试，尚未提交或部署生产；部署后需观察 429、归档错误和 ERP 企微告警数量是否下降。
...
### 2026-08-29 定时任务嵌套确认表单失效与重复文案 — 247 已修复，待部署验证

- 根因是预检通过后的 `scheduled_task_confirm` 被保存为创建表单的 `next_form`，但 245 的 `transition_chat_form_state` 只遍历消息内容的顶层数组；确认提交因此返回“表单不存在或已失效”。
- 迁移 246 的递归路径表达式使用了 PostgreSQL 对 `text[] || unknown` 有歧义的操作符，运行时把 `next_form` 当作数组字面量，确认与取消都会失败；247 改为无歧义的 `array_append(text[], text)`。
- 表单工具结果现在终结本次模型循环，不再由模型在结构化表单后附加重复的“配置表单已生成/任务预览”文字；前端同时隐藏已存量消息中的该已知重复格式。
- 确认成功后数据库草稿、正式任务和嵌套表单状态均正确，但前端仅更新了已挂载卡片的局部状态，未更新对话缓存；切换对话会重新渲染旧缓存。表单结果现同步回写缓存，若缓存已淘汰则标记下一次打开强制读数据库。
- 回归覆盖嵌套状态迁移、表单终结循环、旧消息清理和嵌套确认缓存回写；待部署后验证确认成功后立即切换离开并返回对话，卡片保持“已启用”。

---

### 2026-08-29 定时任务生成失败被误记成功 — 已修复，待生产验证

- 生产主机使用统一层级 cgroup v2，而 `deploy/sandbox.cfg` 强制 nsjail 按 cgroup v1 创建 memory/pids/cpu 控制器，导致代码沙盒在启动时失败；数据查询虽完成，分析与报表生成无法执行。
- nsjail 改为启动时探测 v1/v2，同时保持网络、命名空间、内存、CPU、进程和 rlimit 限制；不允许回退到裸进程。
- 工具循环若未形成最终结论，会把最近工具失败原因传给定时任务；编排器将所有非 `success` 结果作为失败，触发既有退款与重试路径，且不再创建成功投递。
- 受影响的历史运行需按其唯一积分交易执行一次原子补偿；发布后以隔离的沙盒启动探针验证 cgroup v2 实际可用。

---

### 2026-08-29 定时任务企微推送可靠性 — 开发完成，待生产验证

- 根因是定时任务把 Redis Pub/Sub 的发布成功当成企微发送成功；WS Runner 未连接、重连或缺少组织客户端时直接丢弃消息，既无投递事实也无重试。
- 迁移 242 将结果投递改为 `scheduled_task_deliveries` Outbox：任务成功、运行记录和企微待投递事实在一个 RPC 中提交；企微 Worker 使用数据库租约、指数退避和死信状态恢复断线投递。
- “立即执行”改为数据库原子领取，避免与到期扫描重复执行；一次性任务恢复改用 `run_at`，不再对空 `cron_expr` 做计算。
- 定向回归覆盖 Outbox 数据契约、WS 不可用重试、结果目标去重、手动领取竞争和一次性任务恢复；生产发布必须随代码执行 242 迁移并重启 backend 与 `everydayai-wecom`，然后验证断线恢复和死信告警。
