...
单文件下载走 CDN 直连(OSS ObjectMeta 自带 Content-Disposition: attachment),
不需要后端代理。批量必须经后端代理(CDN 无法在线打 ZIP)。
"""
...
async def download_workspace_zip(
    ...
    单文件下载请走 CDN 直连(OSS ObjectMeta 自带 attachment header,前端 a.click 即可)。
    本端点专用于多文件/文件夹的批量打包,CDN 无法在线打 ZIP。
