-- 210: Worker orphan task recovery capability.

SET LOCAL ROLE everydayai_owner;

CREATE OR REPLACE FUNCTION _assert_worker_orphan_recovery_scope()
RETURNS VOID
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
    IF session_user <> 'everydayai_worker'
       ...
       OR NULLIF(current_setting('app.actor_user_id', TRUE), '') IS NOT NULL
       OR NULLIF(current_setting('app.org_id', TRUE), '') IS NOT NULL
       OR NULLIF(current_setting('app.request_id', TRUE), '') IS NULL THEN
        RAISE EXCEPTION 'ORPHAN_RECOVERY_WORKER_SCOPE_REQUIRED'
            USING ERRCODE = '42501';
    END IF;
END;
$$;
    ...
    WITH candidates AS (
        SELECT task.id
         ...
         WHERE task.status IN ('pending', 'running')
           AND NOT (
               task.delivery_context @> '{"actor": true}'::JSONB
           )
           AND (
...
REVOKE ALL ON FUNCTION _assert_worker_orphan_recovery_scope()
