...
CREATE FUNCTION create_agent_child_run(p_parent_run_id UUID,p_parent_action_id UUID,p_parent_request_hash TEXT,p_child_ordinal INTEGER,p_capability TEXT,p_context JSONB) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
...
BEGIN
 ...
 IF $3 !~ '^[0-9a-f]{64}$' OR jsonb_typeof(COALESCE($6,'{}')) IS DISTINCT FROM 'object'
    OR NOT ($6 ? 'policy_receipt_id') OR NOT ($6 ? 'capability') OR NOT ($6 ? 'budget_remaining') THEN
    RAISE EXCEPTION 'AGENT_CHILD_RUN_POLICY_CONTRACT_INVALID';
 END IF;
 IF EXISTS (SELECT 1 FROM agent_runs existing WHERE existing.parent_action_id=a.id AND existing.child_ordinal=$4) THEN
    SELECT id,parent_run_id,parent_action_id,child_ordinal INTO c FROM agent_runs WHERE parent_action_id=a.id AND child_ordinal=$4;
    RETURN jsonb_build_object('outcome','already_exists','child_run_id',c.id,'parent_run_id',c.parent_run_id,'parent_action_id',c.parent_action_id);
 END IF;
