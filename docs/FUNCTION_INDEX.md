...
| `create_agent_runtime_scheduled_execution_profile_v1` / `read_agent_runtime_scheduled_execution_profile_v1` | `backend/migrations/227_29_agent_runtime_scheduled_execution_owner.sql` | Derive the explicit new Runtime task profile only from persisted Session/Command/Run/Action/227_28 facts; freeze model/scope/channel/budget and the exact C7-B3.2-A 9/17-tool safe-read release without accepting Worker snapshots. |
...
| `bind_agent_runtime_scheduled_run_runtime_v1` / owner gate RPCs | `backend/migrations/227_29_agent_runtime_scheduled_execution_owner.sql` | Reject arbitrary same-tenant Commands and require a task/run/trigger/hash/revision-bound scheduler system envelope before unique Runtime binding; private owner gate is reserved for later legacy create/credits/completion wrappers and has no external ACL. |
