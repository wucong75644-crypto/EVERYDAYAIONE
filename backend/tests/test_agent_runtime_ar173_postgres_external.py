...
def test_ar173_226_apply_rollback_reapply_and_worker_acl(database: str) -> None:
    migrations = [f"226_{index:02d}_" for index in range(1, 8)]
...
def test_ar173_worker_rpc_behavior_matrix_and_50_concurrent_idempotency(database: str) -> None:
    ...
    reserve_params = (ids["action"], ids["attempt"], 3, "credits")
    with ThreadPoolExecutor(max_workers=50) as pool:
        outcomes = list(pool.map(lambda _: _worker_rpc(database, "reserve_agent_action_cost", reserve_params), range(50)))
    assert sum(item["outcome"] == "applied" for item in outcomes) == 1
    assert sum(item["outcome"] == "duplicate" for item in outcomes) == 49
    ...
    with ThreadPoolExecutor(max_workers=50) as pool:
        callbacks = list(pool.map(lambda _: _worker_rpc(database, "record_agent_action_callback", callback), range(50)))
    assert sum(item["outcome"] == "accepted" for item in callbacks) == 1
    assert sum(item["outcome"] == "duplicate" for item in callbacks) == 49
    terminal = _worker_rpc(database, "record_agent_action_provider_terminal", (ids["attempt"], ids["token"], ids["request_hash"], "completed", {"result": "ok"}, {}))
    assert terminal["outcome"] == "completed"
    child = _worker_rpc(database, "create_agent_child_run", (ids["run"], ids["action"], ids["request_hash"], 0, "runtime.child", {"policy_receipt_id": ids["policy"], "capability": "runtime.child", "budget_remaining": 1}))
    assert child["outcome"] == "created"
    readback = _worker_rpc(database, "read_agent_child_run", (child["child_run_id"], ids["run"]))
    assert readback["outcome"] == "readback" and readback["status"] == "queued"
    completed_child = _worker_rpc(database, "complete_agent_child_run", (child["child_run_id"], ids["run"], 1, {"items": []}))
    assert completed_child["outcome"] == "completed"
    duplicate_settle = _worker_rpc(database, "settle_agent_action_cost", (ids["action"], ids["attempt"], 3, "credits", "f" * 64))
    assert duplicate_settle["outcome"] == "applied"
