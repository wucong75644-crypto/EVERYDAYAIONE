...
### 2026-07-16 工作区图片插入与聊天附件标准化 — 开发完成，待人工验收
...
**设计结论**：
- 不修改后端 `ContentPart` 协议、API 或数据库；后端已支持 `ImagePart + workspace_path`。
- 在前端聊天提交边界新增纯函数附件标准化，复用现有文件分类和原图 URL 规则。
- 工作区图片生成 `ImagePart`，非图片继续生成 `FilePart`，并贯通聊天、图生图、图生视频和电商图模式。
- 保持现有草稿事务和幂等语义；标准化本地校验必须发生在草稿移出之前。
- `InputControls.tsx` 已超过 500 行，工作区附件预览将拆入独立子组件，不扩大超长文件。

**关联文档**：[TECH_工作区图片插入与聊天附件标准化.md](document/TECH_工作区图片插入与聊天附件标准化.md)

**当前进度**：
- 已新增附件标准化纯函数，工作区图片生成完整 `ImagePart` 元数据，普通文件继续使用 `FilePart`。
- 已贯通聊天、图片生成、视频生成和电商图模式；无有效原图时在草稿移出前阻止媒体生成。
- 已拆出工作区附件预览与 Props 类型，`InputControls.tsx` 从 600 行降至 461 行。
- 专项覆盖：24 passed，语句 94.80%、分支 83.33%、函数 100%。
- 前端全量：104 files / 1138 passed；TypeScript 检查与生产构建通过。
- 后端聊天上下文、消息 Schema 和路由专项：119 passed / 3 skipped。
- 待人工验收工作区右键插入、`@`提及、图文混合和图生图/图生视频流程。
