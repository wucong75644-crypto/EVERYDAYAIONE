...
ERP_ROUTING_PROMPT = (
    ...
    "- 同一编码每会话只识别一次\n\n"
    "### 规则\n"
    "- 禁止猜测参数值，有歧义时停止查询并在回复中向用户确认\n"
    "- 参数明确时直接查询，禁止试探性查询\n"
    "- 名称搜索无结果 → 停止查询，在回复中向用户确认\n"
)
