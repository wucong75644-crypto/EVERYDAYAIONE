...
- 生产证据：19:54 两条独立企微消息均在 Actor 入队前因
  `permission denied for table users` 失败；通讯录解析和活跃记录同时暴露 Actor Scope
  绑定时序错误。
- 根因：角色切换后，普通生成、积分指令、会话设置、新对话和记忆指令仍残留面向旧
  `everydayai` 角色的直表访问。
- 当前状态：代码与迁移 168 已完成能力门面收口并通过本地定向验证；生产在迁移部署及
  真实企微复测发现 `OrgScopedDB` 自动注入 `p_org_id`，而生成上下文首版只有两参数
  签名；迁移 169 增加显式企业校验的三参数重载。再次部署并完成真实消息复测前仍视为
  未恢复。
...
- 第一实施组任务 5.3b.2c-1 已完成代码准备：第二批 owner 脚本固定接管 18 张基础事实表、
  ...
  和最小角色授权。
- 生产迁移总流程已完成文档收口：新增 150–161 总 Runbook，确认生产顺序必须是第一批
  owner → 第二批 owner → 标准 Runner 一次性应用 150–161 → 分服务角色切换 → 旧连接
  归零后最终收口。此前 `DEPLOYMENT.md` 写成第二批 owner 只需在 153 前完成，与迁移
  152 的函数 owner 前置条件冲突，现已纠正为 152 前；同时避免误用不支持 `--through`
  apply 的 Migration Runner。当前仍未连接或修改生产数据库。
- 150–161 生产只读总 preflight 已完成脚本本体：使用安全管理员 psql 启动器和只读
  事务，核验完整/未应用两种账本状态、checksum、角色合同、两批 owner、迁移后 RLS、
...
- 根因修复已实现：共享 RPC 边界统一适配 JSON 参数；迁移 186 将失败退款和媒体 task
  ...
  task 列表计数。生产应用 186 后三类原错误归零，三个旧任务均已进入正确终态且不再
  重复下载；最终消息提交又暴露迁移 171 对历史 VARCHAR task 标识与 UUID JSON 值
  直接比较的类型缺口。迁移 187 上线后恢复事务继续发现 messages.task_id 的 UUID
  写入缺少显式转换；事务完整回滚、消息仍为 pending。完成整函数生产列类型审计后，
  迁移 188 显式转换 role、content、task_id，并在生产 schema 事务预演成功后上线。
- 迁移 186–188 生产账本均为 applied；共享 RPC、退款权限、轮询变量、消息 Scope 和
...
# 2026-07-26 最终撤权后核心链路回归 — 已部署，待真实 UI 联合验收
...
- 提交 `40538066536114ac0e7d8b8406cc24c66c57f187` 已部署，迁移 203 账本状态为
  `applied:migration`；发布全量测试 `8438 passed, 5 skipped, 4 xfailed`，四服务与
  公网健康检查通过。生产 runtime 回滚事务已验证新建对话 `INSERT ... RETURNING`
  和 `prepare_generation` 权限恢复。
- 生产已备份 `.env` 并临时清空 `OSS_CDN_DOMAIN`，新对象 URL 改用阿里云 OSS 官方
  HTTPS 域名；这是对过期 CDN 证书的安全缓解，不是证书根治，旧 CDN URL 仍需续期
  后恢复。Worker 固定 AI Bundle 已可调用，但配置控制面返回
  `CONFIG_REGISTRY_DRIFT`，当前代码按设计降级平台默认 AI Key，企业 BYOK 仍待修复。
...
- 迁移 206 后真实文字生成恢复；图片请求已成功创建 KIE 外部任务，但 Runtime 无权
  ...
  恢复，不直接修改底表。
- 迁移 207 真实图片 E2E 已完成：上传、缩略图、Provider、Worker、消息和 6 积分
  结算均闭环，结果图片 HTTP 200。测试同时发现跨用户会话的幂等领取虽被数据库
  正确拒绝，但原始 `42501` 未映射而返回 HTTP 500；服务现仅将精确
...
# 2026-07-27 异步媒体结果偏离原占位位置 — 本地根因修复，待部署验证

- 根因是媒体任务复用了每个对话唯一的 `streamingMessages` 槽位：后发任务覆盖映射后，
  先发任务响应会删除错误占位符并把结果追加到尾部；WebSocket 终态还会无条件清除
  同一对话的文字流。
- 持久化结果虽然继续使用正确的 `placeholder_message_id` 和原 `created_at`，前端统一
  消息视图在没有新乐观消息时跳过排序，使最终数组退化为任务完成顺序。
- 修复保持文字流单例和媒体 task Map 的既有职责，不改变 API、数据库或后端；媒体
