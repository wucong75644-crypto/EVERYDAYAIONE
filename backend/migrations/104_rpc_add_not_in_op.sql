-- 104: RPC 新增 not_in 操作符 — 支持多值排除过滤
--
-- 两个 RPC 函数的 filter DSL CASE 块均新增 WHEN 'not_in' 分支，
-- 与 'in' 完全对称：NOT IN (SELECT jsonb_array_elements_text(...))。
--
-- 影响: erp_global_stats_query + erp_order_stats_grouped
-- 依赖: 无（纯增量，不改签名/白名单/聚合逻辑）
