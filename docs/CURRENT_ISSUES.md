...
- 2026-09-11 / 05 附件读取：1df0e01c 生产中附件已落盘且进入模型上下文，模型仍误报无附件或搜索上一份文件。已补当前 user 消息中的文件身份绑定；718 项回归、6 次真实模型合成调用通过。候选未部署、原生产复验未关闭，不能宣布根治。证据与验收见 [附件绑定记录](document/TOOL_UNIFICATION_05_ATTACHMENT_BINDING.md)。
